...
| 8     | 🔴 P0    | **Identity & accounts data model** — many wallets and many exchange accounts per user, username (nick) login instead of email, exchange-agnostic credential envelopes, and a bot → exchange-account binding so the engine knows which account to trade. Blocks the Lighter engine work: `/credentials/:botId` can only return one account today. Design + phased plan: [DATA_MODEL.md](DATA_MODEL.md)    | ⬜                           |
