...
async function main(): Promise<void> {
  ...
  try {
    ...
    const appliedResult = await pool.query(
      `SELECT filename, applied_at FROM ${LEDGER_TABLE}`
    );
    ...
    if (applied.size === 0) {
      ...
      if (core.rows[0].exists) {
        ...
        console.log(
          "   Run `npm run db:migrate:files` once to baseline the ledger"
        );
        console.log(
          "   (all current migrations will be marked applied, not re-executed).\n"
        );
      }
    }
    ...
    for (const file of files) {
      ...
      if (appliedAt) {
        console.log(
          `  ✔ ${file.padEnd(38)} applied ${appliedAt.toISOString()}`
        );
      } else {
    ...
    console.log("-".repeat(64));
    console.log(
      `  ${files.length} migration files, ${files.length - pendingCount} applied, ${pendingCount} pending`
    );
  } finally {
    await pool.end();
  }
