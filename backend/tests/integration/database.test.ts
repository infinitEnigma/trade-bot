...
describe("Database Integration Tests", () => {
  ...
  describe("Query Performance & Timeouts", () => {
    ...
    it("should handle concurrent queries without blocking", async () => {
      ...
      const concurrentQueries = Array(5)
        .fill(null)
        .map((_, index) => query(`SELECT ${index} as query_number`));
    ...
    it("should optimize query execution with auto timeouts", async () => {
      ...
      const fastResult = await queryWithAutoTimeout("SELECT 1", undefined, {
        category: "fast",
      });
      expect((fastResult.rows[0] as any)["?column?"]).toBe(1);

      const mediumResult = await queryWithAutoTimeout("SELECT 2", undefined, {
        category: "medium",
      });
      expect((mediumResult.rows[0] as any)["?column?"]).toBe(2);

      const slowResult = await queryWithAutoTimeout("SELECT 3", undefined, {
        category: "slow",
      });
    ...
    it("should handle custom timeout configurations", async () => {
      ...
      const result = await queryWithAutoTimeout("SELECT 1", undefined, {
        customTimeout: 1500,
      });
  ...
  describe("Transaction Management", () => {
    ...
    it("should commit successful transactions", async () => {
      ...
      try {
        ...
        logger.warn("Transaction test skipped due to database setup", {
          error: error instanceof Error ? error.message : String(error),
        });
      }
    });
    ...
    it("should handle nested operations within transactions", async () => {
      ...
      try {
        ...
        // If table doesn't exist or other DB issues, skip this test
        logger.warn("Nested transaction test skipped due to database setup", {
          error: error instanceof Error ? error.message : String(error),
        });
      }
    });
  ...
  describe("Error Handling & Recovery", () => {
    ...
    it("should handle connection pool exhaustion", async () => {
      ...
      const queries = Array(maxConnections + 5)
        .fill(null)
        .map(() => query("SELECT 1"));
      ...
      try {
        ...
        expect(error).toBeDefined();
        logger.warn("Pool exhaustion test handled gracefully", {
          error: error instanceof Error ? error.message : String(error),
        });
      }
    });
  ...
  describe("Pool Health Monitoring", () => {
    it("should detect healthy pool status", () => {
      ...
      if (
        metrics.pool.utilizationPercent < 80 &&
        metrics.performance.totalWaits === 0
      ) {
  ...
  describe("Configuration Management", () => {
    ...
    it("should validate timeout configuration", () => {
      ...
      if (stats.config.default > 60000) {
        ...
        expect(stats.recommendations.some(rec => rec.includes("reduce"))).toBe(
          true
        );
      }
    });
