...
describe("EngineManager", () => {
  ...
  describe("Engine management methods", () => {
    ...
    it("should send start bot command", async () => {
      ...
      // Act
      await engineManager.sendStartBotCommand(
        botId,
        strategyId,
        config,
        credentials
  ...
  describe("Backward compatibility methods", () => {
    ...
    it("should stop engine if no active bots", async () => {
      ...
      const mockKill = jest
        .spyOn((engineManager as any).processSpawner, "kill")
        .mockResolvedValue(undefined);
      ...
      // Assert
      expect(
        mockBotInstanceRepository.getActiveBotInstances
      ).toHaveBeenCalled();
      expect(mockKill).toHaveBeenCalledWith("SIGTERM");
      expect(mockLogger.info).toHaveBeenCalledWith(
        "No active bots, stopping engine"
      );
    });
    ...
    it("should handle error when checking for active bots", async () => {
      // Arrange
      mockBotInstanceRepository.getActiveBotInstances.mockRejectedValue(
    ...
    it("should check if engine process is alive", () => {
      // Arrange
      const mockIsAlive = jest
        .spyOn((engineManager as any).processSpawner, "isAlive")
  ...
  describe("Enterprise supervision methods", () => {
    it("should start and stop process supervision", () => {
      ...
      const mockStartSupervision = jest
        .spyOn((engineManager as any).processSupervisor, "startSupervision")
        .mockImplementation();
      const mockStopSupervision = jest
        .spyOn((engineManager as any).processSupervisor, "stopSupervision")
        .mockImplementation();
    ...
    it("should ensure engine running with supervision", async () => {
      // Arrange
      const mockEnsureEngineRunning = jest
        .spyOn(engineManager, "ensureEngineRunning")
        .mockResolvedValue();
      const mockStartSupervision = jest
        .spyOn(engineManager, "startProcessSupervision")
        .mockImplementation();
    ...
    it("should get supervision status", () => {
      // Arrange
      const mockGetProcessState = jest
        .spyOn((engineManager as any).processSupervisor, "getProcessState")
        .mockReturnValue("running");
      const mockGetState = jest
        .spyOn((engineManager as any).circuitBreaker, "getState")
        .mockReturnValue("closed");
      const mockGetRestartStatistics = jest
        .spyOn((engineManager as any).restartManager, "getRestartStatistics")
        .mockReturnValue({
          totalAttempts: 0,
          nextRetryIn: 0,
        });
      const mockGetRestartAnalysis = jest
        .spyOn((engineManager as any).restartManager, "getRestartAnalysis")
        .mockReturnValue({
          recentAttempts: [],
        });
    ...
    it("should get supervision report", () => {
      ...
      const mockGetSupervisorStatus = jest
        .spyOn((engineManager as any).processSupervisor, "getSupervisorStatus")
    ...
    it("should perform emergency stop", async () => {
      // Arrange
      const mockEmergencyStop = jest
        .spyOn((engineManager as any).processSupervisor, "emergencyStop")
        .mockResolvedValue(undefined);
    ...
    it("should perform manual restart", async () => {
      // Arrange
      const mockManualRestart = jest
        .spyOn((engineManager as any).processSupervisor, "manualRestart")
        .mockResolvedValue({ success: true });
    ...
    it("should reset supervision state", () => {
      ...
      const mockReset = jest
        .spyOn((engineManager as any).processSupervisor, "resetSupervisorState")
        .mockImplementation();
  ...
  describe("Redis stream communication methods", () => {
    ...
    it("should handle errors when publishing commands", async () => {
      ...
      (mockStreamOperations.publish as jest.Mock).mockResolvedValue({
        success: false,
        error: errorMessage,
      });
