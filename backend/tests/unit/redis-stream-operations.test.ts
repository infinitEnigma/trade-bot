...
describe("RedisStreamOperations", () => {
  ...
  describe("Stream operations", () => {
    it("should publish and read a command from a stream", async () => {
      ...
      // Publish command
      const publishResult = await streamOperations.publish(
        "engine:commands",
        command
      );
      ...
      const message = readResult.messages!.find(
        msg =>
    ...
    it("should acknowledge messages in consumer group", async () => {
      ...
      const createGroupResult = await streamOperations.createConsumerGroup(
        TEST_STREAM,
        "test-group"
      );
      ...
      if (readResult.messages && readResult.messages.length > 0) {
        ...
        if (
          ...
          directResult[0].messages.length > 0
        ) {
      ...
      const ackResult = await streamOperations.ack(
        TEST_STREAM,
        "test-group",
        messageId
      );
    ...
    it("should delete a message from stream", async () => {
      ...
      // Delete the message
      const deleteResult = await streamOperations.delete(
        TEST_STREAM,
        messageId
      );
