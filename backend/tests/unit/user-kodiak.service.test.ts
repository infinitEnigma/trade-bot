...
import {
  UserKodiakService,
  createUserKodiakService,
  KodiakUserConfig,
  KodiakCredentials,
} from "../../src/core/user/user-kodiak.service";
import {
  KodiakConnectionStatus,
  KodiakConnectionResult,
} from "../../src/infrastructure/external/kodiak-connection.service";
...
describe("UserKodiakService", () => {
  ...
  describe("linkKodiakAccount", () => {
    ...
    it("should link Kodiak account successfully", async () => {
      ...
      const result = await service.linkKodiakAccount(
        mockUserId,
        mockConnectionData
      );

      expect(result).toEqual(mockResult);
      expect(mockCache.getCachedResult).toHaveBeenCalledWith(
        mockUserId,
        mockConnectionData.accountId
      );
      expect(mockKodiakConnectionService.connectKodiak).toHaveBeenCalledWith(
        mockUserId,
        mockConnectionData
      );
  ...
  describe("unlinkKodiakAccount", () => {
    ...
    it("should unlink Kodiak account successfully", async () => {
      ...
      mockKodiakConnectionService.disconnectKodiak.mockResolvedValue(
        mockResult
      );
      ...
      expect(result).toEqual(mockResult);
      expect(mockKodiakConnectionService.disconnectKodiak).toHaveBeenCalledWith(
        mockUserId
      );
    });
  ...
  describe("getKodiakConnectionStatus", () => {
    ...
    it("should return connection status from cache if valid", async () => {
      ...
      mockKodiakConnectionService.getConnectionStatus.mockResolvedValue(
        mockStatus
      );
      ...
      expect(
        mockKodiakConnectionService.getConnectionStatus
    ...
    it("should throw error when getting connection status fails", async () => {
      ...
      mockKodiakConnectionService.getConnectionStatus.mockRejectedValue(
        testError
      );

      await expect(
        service.getKodiakConnectionStatus(mockUserId)
