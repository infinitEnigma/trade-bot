...
import {
  RedisCacheManager,
  CacheResult,
} from "../../src/infrastructure/cache/redis/cache-manager";
...
describe("RedisCacheManager", () => {
  ...
  describe("atomicCacheUpdate", () => {
    ...
    it("should perform atomic cache update with versioning", async () => {
      ...
      const result = await cacheManager.atomicCacheUpdate(
        "test-key",
        testData,
        "test-key-version"
      );
    ...
    it("should handle atomic cache update failure", async () => {
      ...
      const result = await cacheManager.atomicCacheUpdate("test-key", {
        data: "test",
      });
  ...
  describe("getWithVersion", () => {
    ...
    it("should get cache entry with version", async () => {
      ...
      const result = await cacheManager.getWithVersion(
        "test-key",
        "test-key-version"
      );
  ...
  describe("atomicInvalidate", () => {
    it("should invalidate multiple keys", async () => {
      ...
      const result = await cacheManager.atomicInvalidate(
        ["key1", "key2"],
        "test-reason"
      );
