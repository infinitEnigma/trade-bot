...
import {
  WebSocketError,
  WebSocketErrorCode,
} from "../../src/infrastructure/messaging/websocket/types";

// Mock dependencies
jest.mock("../../src/infrastructure/messaging/websocket/auth");
jest.mock("../../src/infrastructure/messaging/websocket/handlers");
jest.mock(
  "../../src/infrastructure/security/rate-limiter/websocket-rate-limiter.adapter"
);
...
describe("WebSocketService", () => {
  ...
  beforeEach(() => {
    ...
    webSocketService = new WebSocketService(mockAuthService, mockLogger);
  });
  ...
  describe("client management", () => {
    it("should handle client disconnection attempt", () => {
      ...
      (mockServer.sockets!.sockets.get as jest.Mock).mockReturnValue(
        mockSocket
      );
  ...
  describe("health calculation", () => {
    ...
    it("should track response times and metrics", () => {
      ...
      (webSocketService as any).metrics.messagesProcessed =
  ...
  describe("connection event handling", () => {
    ...
    it("should handle market subscribe event", async () => {
      ...
      const marketSubscribeHandler = (
        mockSocket.on as jest.Mock
      ).mock.calls.find((call: any[]) => call[0] === "subscribe_market")?.[1];
    ...
    it("should handle market unsubscribe event", async () => {
      ...
      const marketUnsubscribeHandler = (
        mockSocket.on as jest.Mock
      ).mock.calls.find((call: any[]) => call[0] === "unsubscribe_market")?.[1];
  ...
  describe("authentication and error handling", () => {
    ...
    it("should handle connection errors", () => {
      ...
      const connectionErrorHandler = (
        mockServer.on as jest.Mock
      ).mock.calls.find((call: any[]) => call[0] === "connection_error")[1];
  ...
  describe("metrics tracking", () => {
    ...
    it("should limit response times array size", async () => {
      ...
      if ((webSocketService as any).metrics.responseTimes.length > 1000) {
        (webSocketService as any).metrics.responseTimes = (
          webSocketService as any
        ).metrics.responseTimes.slice(-1000);
      }
  ...
  describe("message handler integration", () => {
    it("should handle successful subscribe operation", async () => {
      ...
      (webSocketService as any).eventHandlers.handleSubscribe =
        mockHandleSubscribe;
    ...
    it("should handle failed subscribe operation", async () => {
      ...
      const mockHandleSubscribe = jest
        .fn()
    ...
    it("should handle successful unsubscribe operation", async () => {
      ...
      (webSocketService as any).eventHandlers.handleUnsubscribe =
    ...
    it("should handle failed unsubscribe operation", async () => {
      ...
      // Mock failed event handler
      const mockHandleUnsubscribe = jest
        .fn()
  ...
  describe("authentication middleware", () => {
    it("should handle authentication error with WebSocketError", async () => {
      ...
      const mockWebSocketError = new WebSocketError(
        "Authentication failed",
        WebSocketErrorCode.AUTHENTICATION_FAILED
      );
      (webSocketService as any).authMiddleware.authenticate = jest
        .fn()
        .mockRejectedValue(mockWebSocketError);
    ...
    it("should handle unexpected authentication error", async () => {
      ...
      (webSocketService as any).authMiddleware.authenticate = jest
        .fn()
        .mockRejectedValue(mockAuthError);
  ...
  describe("event response times", () => {
    it("should track subscribe response time", async () => {
      ...
      expect(
        (webSocketService as any).metrics.responseTimes.length
      ).toBeGreaterThan(0);
    });
    ...
    it("should track market subscribe response time", async () => {
      ...
      (webSocketService as any).eventHandlers.handleMarketSubscribe =
        mockHandleMarketSubscribe;
    ...
    it("should track market unsubscribe response time", async () => {
      ...
      const mockHandleMarketUnsubscribe = jest
        .fn()
        .mockResolvedValue(undefined);
      (webSocketService as any).eventHandlers.handleMarketUnsubscribe =
        mockHandleMarketUnsubscribe;
  ...
  describe("error handling in event handlers", () => {
    ...
    it("should track errors in market subscribe handler", async () => {
      ...
      const mockHandleMarketSubscribe = jest
        .fn()
        .mockRejectedValue(new Error("Market subscribe failed"));
      (webSocketService as any).eventHandlers.handleMarketSubscribe =
        mockHandleMarketSubscribe;
    ...
    it("should track errors in market unsubscribe handler", async () => {
      ...
      const mockHandleMarketUnsubscribe = jest
        .fn()
        .mockRejectedValue(new Error("Market unsubscribe failed"));
      (webSocketService as any).eventHandlers.handleMarketUnsubscribe =
        mockHandleMarketUnsubscribe;
  ...
  describe("response time limiting", () => {
    it("should limit response times array to 1000 entries", async () => {
      ...
      expect((webSocketService as any).metrics.responseTimes.length).toEqual(
        1500
      );
  ...
  describe("service health checks", () => {
    ...
    it("should return warning health status when health score is moderate", () => {
      ...
      const originalCalculateHealthScore = (webSocketService as any)
        .calculateHealthScore;
      (webSocketService as any).calculateHealthScore = jest
        .fn()
        .mockReturnValue(70);
      ...
      (webSocketService as any).calculateHealthScore =
        originalCalculateHealthScore;
    });
