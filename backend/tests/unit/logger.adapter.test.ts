...
import {
  LoggerAdapter,
  loggerAdapter,
} from "../../src/infrastructure/adapters/logger/logger.adapter";
...
jest.mock("../../src/core/logging/context-aware-logger.service", () => {
  const original = jest.requireActual(
    "../../src/core/logging/context-aware-logger.service"
  );
...
describe("LoggerAdapter", () => {
  ...
  describe("Child Logger Creation", () => {
    ...
    it("should create separate child loggers with different contexts", () => {
      ...
      expect((child1 as any).context).toEqual(
        expect.objectContaining({ userId: "123" })
      );
      expect((child2 as any).context).toEqual(
  ...
  describe("Performance Tracking", () => {
    it("should start operations with operation timer", () => {
      ...
      (adapter as any).contextAwareLogger.startOperation.mockReturnValue(
        mockTimer
      );
      ...
      expect(
        (adapter as any).contextAwareLogger.startOperation
      ).toHaveBeenCalledWith(operationName, expect.objectContaining(meta));
