...
const { splitStatements, needsBaseline, selectPendingFiles } = require("../../../scripts/run-migrations") as {
    splitStatements: (sql: string) => string[];
    needsBaseline: (appliedCount: number, hasCoreSchema: boolean) => boolean;
    selectPendingFiles: (files: string[], appliedFilenames: string[]) => string[];
};
...
describe("run-migrations SQL tokenizer", () => {
    describe("splitStatements", () => {
        it("should split simple statements on semicolons", () => {
            ...
            expect(splitStatements(sql)).toEqual(["CREATE TABLE a (id int)", "INSERT INTO a VALUES (1)"]);
        });
        ...
        it("should handle escaped single quotes ('') inside strings", () => {
            ...
            expect(splitStatements(sql)).toEqual([`INSERT INTO t VALUES ('it''s;fine')`, "SELECT 1"]);
        });
        ...
        it("should handle double-quoted identifiers with semicolons", () => {
            ...
            expect(splitStatements(sql)).toEqual([`SELECT "col;name" FROM t`, "SELECT 1"]);
        });
    });
});
...
describe("run-migrations ledger helpers", () => {
    ...
    describe("selectPendingFiles", () => {
        it("should return files not present in the applied ledger, preserving order", () => {
            ...
            expect(selectPendingFiles(files, ["001_a.sql", "003_c.sql"])).toEqual(["002_b.sql"]);
        });
