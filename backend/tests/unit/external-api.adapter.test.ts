...
describe("ExternalApiAdapter", () => {
  ...
  describe("getBalance", () => {
    it("should return success with domain balance when Kodiak API succeeds", async () => {
      ...
      (kodiakIntegrationService.getBalance as jest.Mock).mockResolvedValue(
        mockKodiakResult
      );
    ...
    it("should handle exceptions when calling Kodiak API", async () => {
      (kodiakIntegrationService.getBalance as jest.Mock).mockRejectedValue(
  ...
  describe("getPositions", () => {
    it("should return success with domain positions when Kodiak API succeeds", async () => {
      ...
      (kodiakIntegrationService.getPositions as jest.Mock).mockResolvedValue(
        mockKodiakResult
      );
    ...
    it("should handle exceptions when calling Kodiak API", async () => {
      (kodiakIntegrationService.getPositions as jest.Mock).mockRejectedValue(
        new Error("Timeout")
      );
  ...
  describe("getTrades", () => {
    it("should return success with domain trades when Kodiak API succeeds", async () => {
      ...
      (kodiakIntegrationService.getTrades as jest.Mock).mockResolvedValue(
        mockKodiakResult
      );
    ...
    it("should handle exceptions when calling Kodiak API", async () => {
      (kodiakIntegrationService.getTrades as jest.Mock).mockRejectedValue(
  ...
  describe("getAccountInfo", () => {
    it("should return success with domain account info when Kodiak API succeeds", async () => {
      ...
      (kodiakIntegrationService.getAccountInfo as jest.Mock).mockResolvedValue(
        mockKodiakResult
      );
    ...
    it("should handle exceptions when calling Kodiak API", async () => {
      (kodiakIntegrationService.getAccountInfo as jest.Mock).mockRejectedValue(
      ...
      expect(result.error).toContain(
        "External API account info request failed"
      );
    });
  });
  ...
  describe("testConnectivity", () => {
    it("should return success when Kodiak API connectivity test passes", async () => {
      ...
      ).mockResolvedValue(mockKodiakResult);
  ...
  describe("invalidateUserCache", () => {
    it("should call Kodiak integration service to invalidate cache", async () => {
      (
        kodiakIntegrationService.invalidateUserCache as jest.Mock
      ).mockResolvedValue(undefined);

      await adapter.invalidateUserCache("test-user-id");

      expect(kodiakIntegrationService.invalidateUserCache).toHaveBeenCalledWith(
        "test-user-id"
      );
    });
    ...
    it("should handle exceptions when invalidating cache", async () => {
      (
        kodiakIntegrationService.invalidateUserCache as jest.Mock
      ).mockRejectedValue(new Error("Cache server error"));
  ...
  describe("checkNFTOwnership", () => {
    it("should return false for any wallet and contract address", async () => {
      const result = await adapter.checkNFTOwnership(
        "0x1234567890abcdef",
        "0xabcdef1234567890"
      );
  ...
  describe("checkTokenBalance", () => {
    it("should return false for any wallet, token, and min amount", async () => {
      const result = await adapter.checkTokenBalance(
        "0x1234567890abcdef",
        "0xabcdef1234567890",
        BigInt(1000)
      );
