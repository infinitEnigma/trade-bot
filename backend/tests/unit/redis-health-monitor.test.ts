...
import {
  RedisHealthMonitor,
  redisHealthMonitor,
} from "../../src/infrastructure/security/rate-limiter/redis-health-monitor";
...
describe("RedisHealthMonitor", () => {
  ...
  describe("Health check functionality", () => {
    ...
    it("should handle Redis health check failures", async () => {
      // Arrange
      (redisService.isHealthy as jest.Mock).mockRejectedValue(
  ...
  describe("Health statistics", () => {
    ...
    it("should calculate time since last check", async () => {
      ...
      // Assert
      expect(stats2.secondsSinceLastCheck).toBeGreaterThan(
        stats1.secondsSinceLastCheck
      );
    });
  });
