import {
  ...
  createBotPerformanceService,
  IBotPerformanceRepository,
  BotTrade,
  LegacyBotPerformance,
} from "../../src/core/strategies/bot-performance.service.pure";
import {
  ILogger,
  ICacheService,
  CacheResult,
...
describe("BotPerformanceService", () => {
  ...
  describe("recordTrade", () => {
    ...
    it("should validate trade data and throw errors for invalid inputs", async () => {
      ...
      await expect(service.recordTrade(invalidTrade)).rejects.toThrow(
        "Bot ID and symbol are required"
      );

      const invalidQuantityTrade = { ...mockTrades[0], quantity: 0 };
      await expect(service.recordTrade(invalidQuantityTrade)).rejects.toThrow(
        "Quantity and price must be positive"
      );

      const invalidSideTrade = { ...mockTrades[0], side: "INVALID" as any };
      await expect(service.recordTrade(invalidSideTrade)).rejects.toThrow(
        "Side must be BUY or SELL"
      );

      const invalidTimestampTrade = { ...mockTrades[0], timestamp: 0 };
      await expect(service.recordTrade(invalidTimestampTrade)).rejects.toThrow(
