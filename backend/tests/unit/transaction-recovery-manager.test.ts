...
describe("TransactionRecoveryManager Uncovered Scenarios", () => {
  ...
  describe("circuit breaker functionality", () => {
    it("should return error when circuit breaker is open and not ready to reset", async () => {
      ...
      const recoveryManager = (redisTransactions as any)
    ...
    it("should transition to half-open when circuit breaker timeout passed", async () => {
      ...
      const originalConstructor =
        RedisTransactionsInstance.prototype.constructor;
      jest
        .spyOn(RedisTransactionsInstance.prototype, "constructor")
        .mockImplementation(function (this: any) {
          ...
          (this as any).transactionRecoveryManager.circuitBreakerLastFailure =
            Date.now() - 70000; // 70 seconds ago
        });

      const mockConnectionManager = new RedisConnectionManager();
      jest
        .spyOn(mockConnectionManager, "getClient")
        .mockReturnValue(mockClient);
  ...
  describe("calculate delay methods", () => {
    it("should calculate immediate retry delay correctly", async () => {
      ...
      expect(
        recoveryManager.calculateDelay("immediate", "test-key", 1, {
          maxRetries: 5,
          context: "test",
          priority: "critical",
        })
      ...
      expect(
        recoveryManager.calculateDelay("immediate", "test-key", 5, {
          maxRetries: 5,
          context: "test",
          priority: "critical",
        })
    ...
    it("should calculate exponential backoff delay correctly", async () => {
      ...
      // Test various attempts
      const delay1 = recoveryManager.calculateDelay("backoff", "test-key", 1, {
        maxRetries: 5,
        context: "test",
        priority: "normal",
      });
      const delay2 = recoveryManager.calculateDelay("backoff", "test-key", 2, {
        maxRetries: 5,
        context: "test",
        priority: "normal",
      });
    ...
    it("should calculate circuit breaker delay correctly", async () => {
      ...
      // Test various attempts
      const delay1 = recoveryManager.calculateDelay("circuit", "test-key", 1, {
        maxRetries: 5,
        context: "test",
        priority: "normal",
      });
      const delay2 = recoveryManager.calculateDelay("circuit", "test-key", 2, {
        maxRetries: 5,
        context: "test",
        priority: "normal",
      });
    ...
    it("should calculate adaptive delay based on success rate", async () => {
      ...
      const delayLow = recoveryManager.calculateDelay(
        "adaptive",
        "test-key-low",
        1,
        { maxRetries: 5, context: "test", priority: "normal" }
      );
      const delayMedium = recoveryManager.calculateDelay(
        "adaptive",
        "test-key-medium",
        1,
        { maxRetries: 5, context: "test", priority: "normal" }
      );
      const delayHigh = recoveryManager.calculateDelay(
        "adaptive",
        "test-key-high",
        1,
        { maxRetries: 5, context: "test", priority: "normal" }
  ...
  describe("getRecoveryStats method", () => {
    ...
    it("should include adaptive learning data in recovery stats", async () => {
      ...
      expect(stats.adaptiveLearning.averageSuccessRate).toBeCloseTo(
        (0.75 + 0.45) / 2
      );
    });
