...
describe("KodiakCache", () => {
  ...
  describe("extractUserIdFromKey", () => {
    it("should extract user ID from cache key", () => {
      expect((cache as any).extractUserIdFromKey("positions:user123")).toBe(
        "user123"
      );
      expect((cache as any).extractUserIdFromKey("trades:user456:extra")).toBe(
        "user456"
      );
  ...
  describe("extractEndpointFromKey", () => {
    it("should extract endpoint from cache key", () => {
      expect((cache as any).extractEndpointFromKey("positions:user123")).toBe(
        "positions"
      );
      expect(
        (cache as any).extractEndpointFromKey("trades:user456:extra")
      ).toBe("trades");
