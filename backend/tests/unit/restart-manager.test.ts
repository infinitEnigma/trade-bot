import {
  ...
  RestartConfig,
  RestartResult,
} from "../../src/core/strategies/engine/restart-manager";
...
describe("RestartManager", () => {
  ...
  describe("restart policy checks", () => {
    it("should allow immediate restart with IMMEDIATE policy", async () => {
      ...
      const result =
        await restartManager.attemptIntelligentRestart("test_restart");
    ...
    it("should not allow restart with MANUAL_ONLY policy", async () => {
      const restartManager = new RestartManager({
        policy: RestartPolicy.MANUAL_ONLY,
      });
      const result =
        await restartManager.attemptIntelligentRestart("test_restart");
    ...
    it("should respect max attempts configuration", async () => {
      ...
      const result1 =
        await restartManager.attemptIntelligentRestart("test_restart_1");
      expect(result1.success).toBe(true);
      expect(result1.attemptNumber).toBe(1);

      // Second attempt should succeed
      const result2 =
        await restartManager.attemptIntelligentRestart("test_restart_2");
      expect(result2.success).toBe(true);
      expect(result2.attemptNumber).toBe(2);

      // Third attempt should fail (max attempts exceeded)
      const result3 =
        await restartManager.attemptIntelligentRestart("test_restart_3");
  ...
  describe("restart reason validation", () => {
    it("should allow restart for restartable reasons", () => {
      ...
      expect(
        restartManager.shouldAttemptRestartForReason("process_crash")
      ).toBe(true);
      expect(
        restartManager.shouldAttemptRestartForReason("process_unhealthy")
      ).toBe(true);
      expect(
        restartManager.shouldAttemptRestartForReason("health_check_failed")
      ).toBe(true);
      expect(
        restartManager.shouldAttemptRestartForReason("connection_lost")
      ).toBe(true);
    });
    ...
    it("should not allow restart for non-restartable reasons", () => {
      ...
      expect(
        restartManager.shouldAttemptRestartForReason("manual_shutdown")
      ).toBe(false);
      expect(
        restartManager.shouldAttemptRestartForReason("configuration_error")
      ).toBe(false);
      expect(
        restartManager.shouldAttemptRestartForReason("insufficient_permissions")
      ).toBe(false);
      expect(
        restartManager.shouldAttemptRestartForReason("out_of_memory")
      ).toBe(false);
    });
    ...
    it("should allow restart for unknown reasons", () => {
      ...
      expect(
        restartManager.shouldAttemptRestartForReason("unknown_error")
      ).toBe(true);
      expect(
        restartManager.shouldAttemptRestartForReason("database_timeout")
      ).toBe(true);
    });
  });
  ...
  describe("performRestart behavior", () => {
    ...
    it("should handle failed restart", async () => {
      ...
      const testManager = new TestRestartManager();
      const result =
        await testManager.attemptIntelligentRestart("failed_restart");
  ...
  describe("uncovered cases", () => {
    ...
    it("should have high backoff delay recommendation", async () => {
      ...
      expect(analysis.recommendations).toEqual(
        expect.arrayContaining([
          expect.stringContaining("High average backoff delay"),
        ])
      );
    });
    ...
    it("should handle still in backoff period", async () => {
      ...
      const result =
        await restartManager.attemptIntelligentRestart("second_attempt");
