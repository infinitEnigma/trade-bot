import {
  ...
  createPositionValidatorService,
  PositionValidatorServiceDependencies,
  AccountLimits,
  PositionValidationResult,
} from "../../src/core/strategies/position-validator.service.pure";
import {
  IPositionRepository,
  IUserRepository,
  ICacheService,
...
describe("PositionValidatorService", () => {
  ...
  describe("getAccountLimits", () => {
    ...
    it("should throw error when account info API fails", async () => {
      ...
      await expect(service.getAccountLimits(mockUserId)).rejects.toThrow(
    ...
    it("should log errors when API calls reject", async () => {
      ...
      (mockExternalApi.getAccountInfo as jest.Mock).mockRejectedValue(
        testError
      );
  ...
  describe("validatePositionSize", () => {
    it("should return valid result for position within all limits", async () => {
      const result: PositionValidationResult =
        await service.validatePositionSize(1000, mockSymbol, mockAccountLimits);
    ...
    it("should reject position smaller than minimum notional", async () => {
      const result: PositionValidationResult =
        await service.validatePositionSize(5, mockSymbol, mockAccountLimits);
    ...
    it("should reject position exceeding single position limit", async () => {
      const result: PositionValidationResult =
        await service.validatePositionSize(3000, mockSymbol, mockAccountLimits);
    ...
    it("should reject position exceeding symbol-specific max notional", async () => {
      ...
      const result: PositionValidationResult =
        await service.validatePositionSize(
          6000,
          mockSymbol,
          largeAccountLimits
        );
    ...
    it("should reject position exceeding leverage limit", async () => {
      ...
      const result: PositionValidationResult =
        await service.validatePositionSize(
          ...
          accountLimits,
          1,
          10 // 1000% single position limit (essentially no limit)
        );
  ...
  describe("validateUserPosition", () => {
    it("should validate user position successfully", async () => {
      ...
      const result: PositionValidationResult =
        await service.validateUserPosition(mockUserId, 1000, mockSymbol);
  ...
  describe("calculateAccountLimitsFromPositions", () => {
    it("should calculate account limits from position data", async () => {
      ...
      const result: AccountLimits =
        await service.calculateAccountLimitsFromPositions(
          mockUserId,
          mockPositions
        );
    ...
    it("should use default account info if cache is missing", async () => {
      const mockPositions = [{ positionQty: 0.1, markPrice: 50000 }];
  ...
  describe("getAccountInfoFromCache", () => {
    it("should retrieve account info from cache when available", async () => {
      ...
      const getAccountInfoFromCache = jest.spyOn(
        service as any,
        "getAccountInfoFromCache"
      );
