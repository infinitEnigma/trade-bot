...
const mockedLifecycle = botLifecycleService as jest.Mocked<
  typeof botLifecycleService
>;
...
function mockQueryRouting(opts: {
  ...
  mockedQuery.mockImplementation((sql: string, params?: unknown[]) => {
    ...
    if (
      text.includes("desired_state = 'STOPPED'") &&
      text.includes("actual_state IN ('STARTING', 'RUNNING', 'STOPPING')")
    ) {
      return Promise.resolve({ rows: opts.drift });
    }
    if (
      text.includes("actual_state IN ('STARTING', 'STOPPING')") &&
      text.includes("NOT EXISTS")
    ) {
      return Promise.resolve({ rows: opts.stuck });
    }
    if (
      text.includes("desired_state = 'RUNNING'") &&
      text.includes("actual_state IN ('ERROR', 'UNKNOWN')")
    ) {
...
describe("LifecycleReconciliationService", () => {
  ...
  it("reissues BOT_STOP for desired-stopped drift within budget", async () => {
    ...
    expect(mockedLifecycle.reissueStopForReconciliation).toHaveBeenCalledWith(
      driftBot.id,
  ...
  it("counts failures but keeps sweeping the remaining bots", async () => {
    ...
    mockedLifecycle.reissueStopForReconciliation.mockRejectedValue(
