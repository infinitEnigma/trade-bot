...
describe("ContextAwareLogger Performance Optimization", () => {
  ...
  describe("Context Caching", () => {
    it("should cache context information when context hasn't changed", () => {
      ...
      const checkContextChangeSpy = jest.spyOn(
        logger as any,
        "checkContextChange"
      );
    ...
    it("should invalidate cache when context changes", () => {
      ...
      expect(initialCache.cachedInfo?.correlationId).toBe(
    ...
    it("should handle additional metadata correctly with caching", () => {
      ...
      logger.info("Message with different metadata", {
        customField: "value2",
        requestId: "override",
      });
    ...
    it("should work correctly without context", () => {
      ...
      if (initialContext) {
        ...
        expect((logger as any).contextCache.cachedInfo).toBeDefined();
        expect((logger as any).contextCache.cachedInfo?.component).toBe(
          "test-component"
        );
      }
    });
  ...
  describe("Enhanced Error Handling Features", () => {
    describe("Error Classification", () => {
      it("should classify database errors correctly", () => {
        ...
        expect(classification.errorCode).toBe(
      ...
      it("should classify network errors correctly", () => {
        ...
        expect(classification.errorCode).toBe(
          SharedErrorCodes.SERVICE_UNAVAILABLE
        );
        expect(classification.errorSeverity).toBe("high");
      });
    ...
    describe("Error Code Constants", () => {
      it("should have defined error codes for all categories", () => {
        ...
        expect(SharedErrorCodes.EXTERNAL_SERVICE_ERROR).toBe(
        ...
        expect(SharedErrorCodes.INSUFFICIENT_BALANCE).toBe(
          "INSUFFICIENT_BALANCE"
        );
        expect(SharedErrorCodes.CONNECTION_ERROR).toBe("CONNECTION_ERROR");
        expect(SharedErrorCodes.CONFIGURATION_ERROR).toBe(
          "CONFIGURATION_ERROR"
        );
  ...
  describe("HTTP Logging Method", () => {
    ...
    it("should work with HTTP singleton logger", () => {
      ...
      const {
        httpLogger,
      } = require("../../src/core/logging/context-aware-logger.service");
  ...
  describe("Child Logger Functionality", () => {
    ...
    it("should pass additional metadata to child loggers", () => {
      ...
      expect((childLogger as any).additionalMeta).toEqual({
        parent: "test-component",
      });
    });
  ...
  describe("Generic Log Method", () => {
    it("should handle all log levels correctly", () => {
      ...
      const debugSpy = jest
        ...
        .mockImplementation(jest.fn());
      const httpSpy = jest
        .spyOn(loggerModule.logger, "http")
        .mockImplementation(jest.fn());
