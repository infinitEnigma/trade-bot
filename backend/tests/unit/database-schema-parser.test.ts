...
import {
  ...
  ColumnDefinition,
  CheckConstraint,
} from "../../src/shared/validation/database-schema-parser";
...
describe("DatabaseSchemaParser", () => {
  ...
  describe("SQL Statement Parsing", () => {
    ...
    it("should parse CREATE TABLE with CHECK constraints", () => {
      ...
      const userLevelCheck: CheckConstraint =
        usersTable.checkConstraints["user_level"];
      expect(userLevelCheck.expression).toContain("user_level IN");
      expect(userLevelCheck.values).toEqual([
        "BASIC",
        "VERIFIED",
        "PREMIUM",
        "ADMIN",
    ...
    it("should parse ALTER TABLE statements correctly", () => {
      ...
      expect(initialSchema.tables["users"].columns["age"].type).toEqual(
        "INTEGER"
      );
    });
    ...
    it("should extract foreign key relationships", () => {
      ...
      expect(
        schema.tables["strategies"].foreignKeys["user_id"].referencedTable
      ).toEqual("users");
      expect(
        schema.tables["strategies"].foreignKeys["user_id"].referencedColumn
      ).toEqual("id");
    });
  });
  ...
  describe("Check Constraint Parsing", () => {
    it("should parse IN constraints", () => {
      ...
      const constraint = parseCheck(
        "user_level IN ('BASIC', 'VERIFIED', 'PREMIUM', 'ADMIN')"
      );

      expect(constraint).not.toBeNull();
      expect(constraint?.expression).toContain("user_level IN");
      expect(constraint?.values).toEqual([
        "BASIC",
        "VERIFIED",
        "PREMIUM",
        "ADMIN",
  ...
  describe("Real-World Migration Parsing", () => {
    it("should parse existing migration files correctly", async () => {
      ...
      logger.info("Tables found in schema:", {
