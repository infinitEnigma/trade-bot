...
import {
  ...
  BalanceServiceDependencies,
  LegacyBalanceFormat,
} from "../../src/core/wallet/balance.service.pure";
...
describe("BalanceService", () => {
  ...
  describe("getUserBalance", () => {
    ...
    it("should fetch balance from API and cache when cache miss (modern format)", async () => {
      ...
      (deps.externalApi.getBalance as jest.Mock).mockResolvedValue({
        success: true,
        data: mockBalance,
      });
    ...
    it("should return legacy format when LEGACY_BALANCE_API flag is true", async () => {
      ...
      expect(
        Date.parse((result as LegacyBalanceFormat).timestamp)
      ).not.toBeNaN();
  ...
  describe("invalidateBalanceCache", () => {
    it("should invalidate balance cache successfully", async () => {
      ...
      (deps.externalApi.invalidateUserCache as jest.Mock).mockResolvedValue(
        undefined
      );
      ...
      expect(deps.externalApi.invalidateUserCache).toHaveBeenCalledWith(
        testUserId
      );
      expect(deps.logger.info).toHaveBeenCalled();
    });
  ...
  describe("canWithdraw", () => {
    it("should return true when user has sufficient balance to withdraw", async () => {
      ...
      const canWithdrawSpy = jest
        .spyOn(Balance.prototype, "canWithdraw")
  ...
  describe("getBalanceUtilization", () => {
    it("should return correct utilization percentage", async () => {
      ...
      const getUtilizationPercentageSpy = jest
        .spyOn(Balance.prototype, "getUtilizationPercentage")
        .mockReturnValue(20);
      const mockBalance = createMockBalance();
    ...
    it("should throw error when getting balance fails", async () => {
      ...
      await expect(
        balanceService.getBalanceUtilization(testUserId)
      ).rejects.toThrow();
    });
  });
  ...
  describe("Internal Methods", () => {
    ...
    describe("convertToLegacyFormat", () => {
      it("should correctly convert modern balance format to legacy format", () => {
        const convertToLegacyFormat = (service: any, balance: Balance) =>
          service.convertToLegacyFormat(balance);
