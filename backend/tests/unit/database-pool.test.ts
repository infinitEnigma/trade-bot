...
describe("Database Pool - Timeout Configuration Health Checks", () => {
  ...
  describe("getTimeoutStats", () => {
    ...
    it("should detect issue when default timeout is too high (> 60s)", () => {
      ...
      expect(stats.health.issues).toContain(
        "Default timeout > 60s may allow runaway queries"
      );
      expect(stats.recommendations).toContain(
        "Consider reducing default timeout to 30s"
      );
    });
    ...
    it("should detect issue when fast timeout is too high (> 10s)", () => {
      ...
      expect(stats.health.status).toBe("warning");
      expect(stats.health.issues).toContain(
        "Fast query timeout > 10s defeats the purpose"
      );
      expect(stats.recommendations).toContain(
        "Fast queries should timeout < 10s"
      );
    });
    ...
    it("should detect issue when report timeout is too low (< 60s)", () => {
      ...
      expect(stats.health.status).toBe("warning");
      expect(stats.health.issues).toContain(
        "Report timeout < 60s may interrupt long-running reports"
      );
      expect(stats.recommendations).toContain(
        "Consider increasing report timeout to 5+ minutes"
      );
    });
