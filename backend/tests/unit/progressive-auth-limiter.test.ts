...
import {
  ...
  progressiveAuthLimiter,
} from "../../src/infrastructure/security/rate-limiter/progressive-auth-limiter";
...
describe("ProgressiveAuthLimiter", () => {
  ...
  describe("Redis operation failures", () => {
    ...
    it("should handle Redis errors when recording failure", async () => {
      ...
      // Act & Assert
      await expect(
        progressiveAuthLimiter.recordFailure(identifier)
    ...
    it("should handle Redis errors when recording success", async () => {
      ...
      // Act & Assert
      await expect(
        progressiveAuthLimiter.recordSuccess(identifier)
      ).resolves.not.toThrow();
    });
    ...
    it("should handle Redis errors when getting failure info", async () => {
      ...
      // Act & Assert
      await expect(
        progressiveAuthLimiter.getFailureInfo(identifier)
      ).resolves.not.toThrow();
    });
  });
});
