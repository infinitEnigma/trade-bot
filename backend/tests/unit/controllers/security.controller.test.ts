...
jest.mock(
  "../../../src/infrastructure/security/database-security.service",
  () => ({
...
// Get mock services
const mockDatabaseSecurityService =
  require("../../../src/infrastructure/security/database-security.service").databaseSecurityService;
const mockEncryptionService =
  require("../../../src/infrastructure").encryptionService;
...
function createTestApp(): Express {
  ...
  const {
    securityRoutes,
  } = require("../../../src/interfaces/http/system/security");
...
describe("Security Controller", () => {
  ...
  describe("Unauthenticated requests", () => {
    ...
    beforeEach(() => {
      ...
      auth.authMiddleware.mockImplementation(
    ...
    it("should return 401 for GET /assessment without user", async () => {
      const response = await request(app).get("/assessment").expect(401);
    ...
    it("should return 401 for GET /audit-report without user", async () => {
      const response = await request(app).get("/audit-report").expect(401);
    ...
    it("should return 401 for GET /migration-plan without user", async () => {
      const response = await request(app).get("/migration-plan").expect(401);
    ...
    it("should return 401 for POST /rotate-keys without user", async () => {
      const response = await request(app).post("/rotate-keys").expect(401);
  ...
  describe("Authenticated requests", () => {
    describe("GET /assessment", () => {
      it("should return security assessment when authenticated", async () => {
        ...
        mockDatabaseSecurityService.assessDatabaseSecurity.mockResolvedValue(
          mockAssessment
        );

        const response = await request(app).get("/assessment").expect(200);
      ...
      it("should handle errors when retrieving security assessment", async () => {
        ...
        mockDatabaseSecurityService.assessDatabaseSecurity.mockRejectedValue(
          new Error(errorMessage)
        );

        const response = await request(app).get("/assessment").expect(500);

        expect(response.body.success).toBe(false);
        expect(response.body.error).toBe(
          "Failed to generate security assessment"
        );
      });
    });
  });
  ...
  describe("GET /metrics", () => {
    it("should return security metrics when authenticated", async () => {
      ...
      mockDatabaseSecurityService.getSecurityMetrics.mockResolvedValue(
        mockMetrics
      );
    ...
    it("should handle errors when retrieving security metrics", async () => {
      ...
      mockDatabaseSecurityService.getSecurityMetrics.mockRejectedValue(
        new Error(errorMessage)
      );
  ...
  describe("GET /audit-report", () => {
    it("should return security audit report when authenticated", async () => {
      ...
      mockDatabaseSecurityService.generateSecurityAuditReport.mockResolvedValue(
        mockReport
      );

      const response = await request(app).get("/audit-report").expect(200);

      expect(response.text).toEqual(mockReport);
      expect(response.headers["content-type"]).toContain("text/plain");
      expect(response.headers["content-disposition"]).toContain(
        'attachment; filename="security-audit-'
      );
    });

    it("should handle errors when generating security audit report", async () => {
      const errorMessage = "Report generation failed";
      mockDatabaseSecurityService.generateSecurityAuditReport.mockRejectedValue(
        new Error(errorMessage)
      );

      const response = await request(app).get("/audit-report").expect(500);

      expect(response.body.success).toBe(false);
      expect(response.body.error).toBe(
        "Failed to generate security audit report"
      );
    });
  });
  ...
  describe("GET /migration-plan", () => {
    it("should return encryption migration plan when authenticated", async () => {
      ...
      mockDatabaseSecurityService.generateEncryptionMigrationPlan.mockResolvedValue(
        mockMigrationPlan
      );

      const response = await request(app).get("/migration-plan").expect(200);
    ...
    it("should handle errors when generating migration plan", async () => {
      ...
      mockDatabaseSecurityService.generateEncryptionMigrationPlan.mockRejectedValue(
        new Error(errorMessage)
      );

      const response = await request(app).get("/migration-plan").expect(500);
  ...
  describe("POST /migrate-table", () => {
    it("should migrate table encryption when valid data is provided", async () => {
      ...
      mockDatabaseSecurityService.migrateTableEncryption.mockResolvedValue(
        mockResult
      );
    ...
    it("should return 400 when missing required fields", async () => {
      ...
      expect(response.body.error).toContain(
        "tableName (string) and columns (array) are required"
      );
    });
    ...
    it("should handle errors during table migration", async () => {
      ...
      mockDatabaseSecurityService.migrateTableEncryption.mockRejectedValue(
        new Error(errorMessage)
      );
  ...
  describe("POST /enable-encryption", () => {
    it("should enable database encryption when successful", async () => {
      ...
      mockDatabaseSecurityService.enableDatabaseEncryption.mockResolvedValue(
        mockResult
      );
    ...
    it("should handle errors when enabling encryption", async () => {
      ...
      mockDatabaseSecurityService.enableDatabaseEncryption.mockRejectedValue(
        new Error(errorMessage)
      );
  ...
  describe("POST /rotate-keys", () => {
    ...
    it("should return message when key rotation not needed", async () => {
      ...
      expect(response.body.success).toBe(true);
      expect(response.body.message).toEqual(
        "Key rotation not needed at this time"
      );
    ...
    it("should handle errors during key rotation", async () => {
      ...
      mockEncryptionService.isKeyRotationNeeded.mockRejectedValue(
        new Error(errorMessage)
      );

      const response = await request(app).post("/rotate-keys").expect(500);
