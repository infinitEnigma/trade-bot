...
jest.mock(
  "../../../src/infrastructure/security/key-management.service",
  () => ({
...
// Get mock services
const mockGetHealthService =
  require("../../../src/core/service-provider").getHealthService;
...
const mockKeyManagementService =
  require("../../../src/infrastructure/security/key-management.service").keyManagementService;
...
function createTestApp(): Express {
  ...
  const {
    healthRoutes,
  } = require("../../../src/interfaces/http/system/health");
...
describe("Health Controller", () => {
  ...
  describe("GET /metrics/database", () => {
    it("should return database metrics", async () => {
      ...
      const response = await request(app).get("/metrics/database").expect(200);
    ...
    it("should handle metrics retrieval failure", async () => {
      ...
      const response = await request(app).get("/metrics/database").expect(500);
  ...
  describe("GET /health/encryption", () => {
    it("should return encryption health check", async () => {
      ...
      const response = await request(app).get("/health/encryption").expect(200);
    ...
    it("should handle encryption check failure", async () => {
      ...
      const response = await request(app).get("/health/encryption").expect(503);
  ...
  describe("GET /health/redis", () => {
    ...
    it("should return 503 when Redis is disconnected", async () => {
      ...
      const response = await request(app).get("/health/redis").expect(503);
  ...
  describe("GET /health/external", () => {
    it("should return external API health check when Kodiak API is available", async () => {
      const mockResponse = {
        ...
        json: jest
          .fn()
          .mockResolvedValue({ data: { symbol: "PERP_BTC_USDC" } }),
      };
    ...
    it("should return degraded status when Kodiak API fails", async () => {
      ...
      const response = await request(app).get("/health/external").expect(503);
  ...
  describe("GET /health/services", () => {
    it("should return service status information with all services pure and enabled", async () => {
      const response = await request(app).get("/health/services").expect(200);
      ...
      expect(response.body.summary.migrationProgress).toBe(
        "6/6 services migrated"
      );
    });
  });
  ...
  describe("GET /ratelimit", () => {
    it("should return rate limit statistics", async () => {
      ...
      const response = await request(app).get("/ratelimit").expect(200);
    ...
    it("should handle rate limit stats retrieval failure", async () => {
      ...
      const response = await request(app).get("/ratelimit").expect(500);
