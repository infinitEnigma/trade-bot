...
function createTestApp(): Express {
  ...
  const {
    userKodiakRoutes,
  } = require("../../../src/interfaces/http/users/kodiak");
  ...
  describe("Kodiak Connection Routes", () => {
    describe("POST /api/user/kodiak/connect", () => {
      it("should handle unauthenticated user", async () => {
        ...
        jest.doMock(
          "../../../src/interfaces/middleware/auth.middleware",
          () => ({
            authMiddleware: jest
              .fn()
              .mockImplementation((req: any, res: any, next: any) => {
        ...
        expect(response.body.error).toEqual(
          "Failed to connect Kodiak credentials"
        );
      });
      ...
      it("should connect Kodiak account with valid credentials", async () => {
        ...
        serviceProvider
          ...
          .linkKodiakAccount.mockResolvedValue(mockResult);
        ...
        expect(response.body.message).toEqual(
          "Kodiak account connected successfully"
        );
        expect(response.body.data).toEqual({ accountId: "kodiak-account-1" });
        expect(
          serviceProvider.getUserKodiakService().linkKodiakAccount
      ...
      it("should handle errors when connecting Kodiak account", async () => {
        ...
        serviceProvider
          .getUserKodiakService()
          .linkKodiakAccount.mockRejectedValue(new Error("Connection failed"));
    ...
    describe("DELETE /api/user/kodiak/disconnect", () => {
      it("should disconnect Kodiak account", async () => {
        ...
        serviceProvider
          .getUserKodiakService()
          .unlinkKodiakAccount.mockResolvedValue(mockResult);
        ...
        expect(response.body.success).toBe(true);
        expect(response.body.message).toEqual(
          "Kodiak account disconnected successfully"
        );
        expect(
          serviceProvider.getUserKodiakService().unlinkKodiakAccount
      ...
      it("should handle errors when disconnecting Kodiak account", async () => {
        ...
        serviceProvider
          .getUserKodiakService()
          .unlinkKodiakAccount.mockRejectedValue(
        ...
        expect(response.body.success).toBe(false);
        expect(response.body.error).toEqual(
          "Failed to disconnect Kodiak credentials"
        );
      });
    });
    ...
    describe("GET /api/user/kodiak/status", () => {
      it("should return Kodiak connection status", async () => {
        ...
        serviceProvider
          .getUserKodiakService()
          .getKodiakConnectionStatus.mockResolvedValue(mockStatus);
        ...
        expect(
          serviceProvider.getUserKodiakService().getKodiakConnectionStatus
        ).toHaveBeenCalledWith("user-123");
      });
      ...
      it("should handle errors when getting Kodiak status", async () => {
        ...
        serviceProvider
          .getUserKodiakService()
          .getKodiakConnectionStatus.mockRejectedValue(
  ...
  describe("Kodiak Data Routes", () => {
    describe("GET /api/user/kodiak/positions", () => {
      it("should return Kodiak positions", async () => {
        ...
        const kodiakIntegrationService =
        ...
        expect(kodiakIntegrationService.getPositions).toHaveBeenCalledWith(
          "user-123"
        );
      });
      ...
      it("should handle errors when getting Kodiak positions", async () => {
        const kodiakIntegrationService =
          require("../../../src/infrastructure/external/kodiak-integration.service").kodiakIntegrationService;
        kodiakIntegrationService.getPositions.mockRejectedValue(
    ...
    describe("GET /api/user/kodiak/trades", () => {
      it("should return Kodiak trades with default limit", async () => {
        ...
        expect(kodiakIntegrationService.getTrades).toHaveBeenCalledWith(
          "user-123",
          50
        );
      });
      ...
      it("should handle errors when getting Kodiak trades", async () => {
        const kodiakIntegrationService =
          require("../../../src/infrastructure/external/kodiak-integration.service").kodiakIntegrationService;
        kodiakIntegrationService.getTrades.mockRejectedValue(
          new Error("Network error")
        );
    ...
    describe("GET /api/user/kodiak/balance", () => {
      it("should return Kodiak balance", async () => {
        ...
        expect(kodiakIntegrationService.getBalance).toHaveBeenCalledWith(
          "user-123"
        );
      });
      ...
      it("should handle errors when getting Kodiak balance", async () => {
        const kodiakIntegrationService =
          require("../../../src/infrastructure/external/kodiak-integration.service").kodiakIntegrationService;
        kodiakIntegrationService.getBalance.mockRejectedValue(
          new Error("Network error")
        );
    ...
    describe("GET /api/user/kodiak/account-info", () => {
      it("should return Kodiak account info", async () => {
        ...
        expect(kodiakIntegrationService.getAccountInfo).toHaveBeenCalledWith(
          "user-123"
        );
      });
      ...
      it("should handle errors when getting Kodiak account info", async () => {
        const kodiakIntegrationService =
          require("../../../src/infrastructure/external/kodiak-integration.service").kodiakIntegrationService;
        kodiakIntegrationService.getAccountInfo.mockRejectedValue(
          new Error("Network error")
        );
        ...
        expect(response.body.success).toBe(false);
        expect(response.body.error).toEqual(
          "Failed to get Kodiak account info"
        );
      });
    });
  });
  ...
  describe("Public Kodiak Routes", () => {
    describe("GET /api/user/public/kodiak/availability", () => {
      ...
      it("should handle errors when checking Kodiak availability", async () => {
        ...
        const logger =
          require("../../../src/core/logging/logger.service").default;
        ...
        const testApp = (() => {
          ...
          const app = express();
          app.get(
            "/api/user/public/kodiak/availability",
