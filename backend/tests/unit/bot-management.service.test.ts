...
import {
  ...
  createBotManagementService,
  BotManagementServiceDependencies,
} from "../../src/core/bots/bot-management.service";
...
describe("BotManagementService", () => {
  ...
  describe("Get Bot Instances", () => {
    it("should retrieve bot instances for a user", async () => {
      ...
      (
        deps.botInstanceRepository.getBotInstances as jest.Mock
      ...
      expect(deps.botInstanceRepository.getBotInstances).toHaveBeenCalledWith(
        testUserId
      );
    ...
    it("should handle errors when getting bot instances", async () => {
      ...
      await expect(
        botManagementService.getBotInstances(testUserId)
      ).rejects.toThrow("Failed to get bot instances");
  ...
  describe("Get Bot Instance", () => {
    ...
    it("should handle errors when getting a bot instance", async () => {
      ...
      await expect(
        botManagementService.getBotInstance(testBotId)
      ).rejects.toThrow("Failed to get bot instance");
      expect(deps.logger.error).toHaveBeenCalled();
    });
  });
  ...
  describe("Create and Start Bot", () => {
    it("should create and start a new bot instance", async () => {
      ...
      (
        deps.botInstanceRepository.createBotInstance as jest.Mock
      ).mockResolvedValue(mockBotInstance);
      (deps.auditLogRepository.logEvent as jest.Mock).mockResolvedValue(
        undefined
      );

      const result = await botManagementService.createAndStartBot(
        testUserId,
        testStrategyId,
        testNotionalAmount
      );
    ...
    it("should throw error if strategy does not belong to user", async () => {
      ...
      await expect(
        botManagementService.createAndStartBot(
          testUserId,
          testStrategyId,
          testNotionalAmount
        )
    ...
    it("should throw error if bot is already running for strategy", async () => {
      ...
      const mockActiveBots = [
        { id: "bot-789", strategy_id: testStrategyId, status: "RUNNING" },
      ];
      ...
      (
        deps.botInstanceRepository.getActiveBotInstances as jest.Mock
      ).mockResolvedValue(mockActiveBots);
  ...
  describe("Stop Bot", () => {
    it("should stop a running bot instance", async () => {
      ...
      (
        deps.botInstanceRepository.updateBotStatus as jest.Mock
      ...
      expect(deps.botInstanceRepository.updateBotStatus).toHaveBeenCalledWith(
        testBotId,
        "STOPPED"
      );
      expect(deps.auditLogRepository.logEvent).toHaveBeenCalled();
    ...
    it("should throw error if bot does not belong to user", async () => {
      ...
      await expect(
        botManagementService.stopBot(testUserId, testBotId)
  ...
  describe("Get Bot Performance", () => {
    ...
    it("should throw error if bot not found", async () => {
      ...
      await expect(
        botManagementService.getBotPerformance(testBotId)
      ).rejects.toThrow("Failed to get bot performance");
      expect(deps.logger.error).toHaveBeenCalled();
    });
  });
