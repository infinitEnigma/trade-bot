import {
  ...
  ProcessConfig,
  ReadinessConfig,
} from "../../src/core/strategies/engine/process-spawner";
...
describe("ProcessSpawner", () => {
  ...
  describe("spawn", () => {
    it("should reject if process spawning fails", async () => {
      ...
      await expect(processSpawner.spawn()).rejects.toThrow(
    ...
    it("should reject if process fails to start within timeout", async () => {
      ...
      await expect(processSpawner.spawn()).rejects.toThrow(
        "Process failed to start within timeout"
      );
    });
  ...
  describe("waitForReady", () => {
    it("should reject if process fails to become ready", async () => {
      ...
      await expect(
        processSpawner.waitForReady({ maxAttempts: 2, attemptInterval: 10 })
      ).rejects.toThrow("Engine failed to become ready");
