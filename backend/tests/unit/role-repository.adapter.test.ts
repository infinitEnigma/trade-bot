...
import {
  ...
  roleRepositoryAdapter,
} from "../../src/infrastructure/adapters/repositories/role-repository.adapter";
...
describe("RoleRepositoryAdapter", () => {
  ...
  describe("assignRole", () => {
    ...
    it("should log error and rethrow when query fails", async () => {
      ...
      await expect(adapter.assignRole(userId, role, grantedBy)).rejects.toThrow(
        testError
      );
  ...
  describe("getUserRoles", () => {
    it("should return user roles", async () => {
      (query as jest.Mock).mockResolvedValue({
        rows: [{ role: UserRole.QUALIFIED_ALPHA }],
      });
      const adapter = new RoleRepositoryAdapter();
      const userId = "test-user-id";
