...
import {
  ...
  auditLogRepositoryAdapter,
} from "../../src/infrastructure/adapters/repositories/audit-log-repository.adapter";
...
describe("AuditLogRepositoryAdapter", () => {
  describe("Initialization", () => {
    ...
    it("should export a singleton instance", () => {
      expect(auditLogRepositoryAdapter).toBeInstanceOf(
        AuditLogRepositoryAdapter
      );
    });
  });
  ...
  describe("getUserLogs", () => {
    ...
    it("should handle limit parameter correctly", async () => {
      ...
      expect(query).toHaveBeenCalledWith(expect.anything(), [userId, limit]);
    });
  });
  ...
  describe("Legacy Methods", () => {
    ...
    describe("getSecurityEvents", () => {
      it("should retrieve security events in legacy format", async () => {
        ...
        const events = await adapter.getSecurityEvents(
          startDate,
          endDate,
          1000
        );
      ...
      it("should throw error when security events query fails", async () => {
        ...
        await expect(
          adapter.getSecurityEvents(startDate, endDate)
        ).rejects.toThrow(
