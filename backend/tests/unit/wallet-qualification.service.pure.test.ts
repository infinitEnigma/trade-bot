...
import {
  ...
  QualificationResult,
} from "../../src/core/wallet/wallet-qualification.service.pure";
...
describe("WalletQualificationService", () => {
  ...
  describe("Constructor", () => {
    it("should create an instance of WalletQualificationService", () => {
      ...
      expect(walletQualificationService).toBeInstanceOf(
        WalletQualificationService
      );
    });
  ...
  describe("checkAlphaQualification", () => {
    it("should return disqualified when user has no wallet address", async () => {
      ...
      const result =
        await walletQualificationService.checkAlphaQualification(testUserId);
    ...
    it("should return disqualified when wallet is on incorrect chain", async () => {
      ...
      (deps.externalApi.validateWalletChain as jest.Mock).mockResolvedValue(
        false
      );
      ...
      expect(result.reasons).toEqual(
        expect.arrayContaining([
          "Wallet not connected to required chain (Base/8453)",
        ])
      );
    ...
    it("should return qualified when user owns the required NFT", async () => {
      ...
      (deps.externalApi.checkTokenBalance as jest.Mock).mockResolvedValue(
        false
      );
    ...
    it("should return qualified when user has minimum token balance", async () => {
      ...
      (deps.externalApi.checkNFTOwnership as jest.Mock).mockResolvedValue(
        false
      );
    ...
    it("should handle errors gracefully", async () => {
      ...
      (deps.externalApi.validateWalletChain as jest.Mock).mockRejectedValue(
        new Error("API Error")
      );
      ...
      expect(result.criteria.tokens).toEqual([]);
      expect(result.reasons).toEqual([
        "Qualification check failed due to system error",
      ]);
    ...
    it("should use AND logic when configuration is set to AND", async () => {
      ...
      const originalLogic = (
        await import("../../src/core/wallet/wallet-qualification.service.pure")
      ).ALPHA_QUALIFICATION_CONFIG.logic;
      (
        await import("../../src/core/wallet/wallet-qualification.service.pure")
      ).ALPHA_QUALIFICATION_CONFIG.logic = "AND";
      ...
      (
        await import("../../src/core/wallet/wallet-qualification.service.pure")
      ).ALPHA_QUALIFICATION_CONFIG.logic = originalLogic;
    });
  });
  ...
  describe("getQualificationConfig", () => {
    ...
    it("should return configuration with correct contract addresses", () => {
      ...
      const nftRequirement = config.requirements.find(
        (req: any) => req.type === "nft"
      );
      ...
      const tokenRequirement = config.requirements.find(
        (req: any) => req.type === "token"
      );
