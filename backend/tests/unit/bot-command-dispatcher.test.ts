...
import { EngineProtocolService, SendCommandResult } from '../../src/core/bots/engine-protocol.service';
...
describe('BotCommandDispatcher (record-before-publish)', () => {
    ...
    beforeEach(() => {
        repository = {
            ...
            findStrategyConfig: jest.fn().mockResolvedValue({ symbol: 'PERP_BTC_USDC', gridSize: 10 }),
        ...
        repository.findStrategyConfig = jest.fn().mockResolvedValue({ symbol: 'PERP_BTC_USDC', gridSize: 10 });
        ...
        engineProtocol.sendCommand = jest.fn().mockImplementation(
    ...
    describe('BOT_START', () => {
        it('records the command as PENDING before publishing, and reuses correlationId', async () => {
            const result = await dispatcher.sendStartCommand('bot-1', 'user-1', 'strat-1');
        ...
        it('marks the pending command FAILED when the publish fails', async () => {
            ...
            engineProtocol.sendCommand.mockImplementation(
                (_type: string, _payload: unknown, correlationId?: string) =>
                    Promise.resolve({ success: false, error: 'connection refused', correlationId })
            );
            ...
            const [correlationId, state, errorCode] = repository.resolveCommand.mock.calls[0];
    ...
    describe('BOT_STOP', () => {
        it('records a pending BOT_STOP before publishing with the shared correlationId', async () => {
            const sendResult: SendCommandResult = { success: true, messageId: 'm-stop', correlationId: 'corr-stop' };
