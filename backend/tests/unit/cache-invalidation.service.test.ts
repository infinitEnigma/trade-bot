...
import {
  ...
  cacheInvalidationService,
} from "../../src/infrastructure/cache/cache-invalidation.service";
...
describe("CacheInvalidationService", () => {
  ...
  describe("setSocketServer", () => {
    it("should set the Socket.IO server instance", () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        "Cache invalidation service initialized with Socket.IO"
      );
    });
  });
  ...
  describe("broadcastInvalidation", () => {
    ...
    it("should log warning and return when Socket.IO not available", async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        "Socket.IO not available for cache invalidation broadcast"
      );
  ...
  describe("broadcastRefresh", () => {
    ...
    it("should log warning and return when Socket.IO not available", async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        "Socket.IO not available for cache refresh broadcast"
      );
      expect(mockIo.emit).not.toHaveBeenCalled();
    });
  });
  ...
  describe("broadcastClear", () => {
    ...
    it("should log warning and return when Socket.IO not available", async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        "Socket.IO not available for cache clear broadcast"
      );
      expect(mockIo.emit).not.toHaveBeenCalled();
    });
  });
  ...
  describe("invalidateByType", () => {
    it("should invalidate market data keys when dataType is market_data", async () => {
      ...
      const calledKeys = mockInvalidateWithBroadcast.mock
        .calls[0][0] as string[];
    ...
    it("should invalidate balance data keys when dataType is balance_data", async () => {
      ...
      const calledKeys = mockInvalidateWithBroadcast.mock
        .calls[0][0] as string[];
      expect(calledKeys).toEqual([CACHE_KEYS.balance(userId)]);
    });
  });
  ...
  describe("invalidateAllMarketData", () => {
    it("should broadcast general market data invalidation event", async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith("Invalidating all market data", {
        reason,
      });
      expect(mockIo.emit).toHaveBeenCalledWith(
        "cache:invalidation",
  ...
  describe("handleCacheEvent", () => {
    it("should handle INVALIDATED cache event", async () => {
      ...
      expect(mockBroadcastInvalidation).toHaveBeenCalledWith(
        event.keys,
        event.reason,
        event.userId
      );
    ...
    it("should handle REFRESHED cache event", async () => {
      ...
      expect(mockBroadcastRefresh).toHaveBeenCalledWith(
        event.keys,
        event.userId
      );
    });
    ...
    it("should handle CLEARED cache event", async () => {
      ...
      expect(mockBroadcastClear).toHaveBeenCalledWith(
        event.pattern,
        event.keysCleared,
        event.userId
      );
    ...
    it("should log warning for unknown cache event type", async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith("Unknown cache event type", {
        event: unknownEvent,
      });
    });
  });
