import {
  BotStatusService,
  createBotStatusService,
  IBotRepository,
  Bot,
  BotWithValidation,
  LegacyBotStatusInfo,
} from "../../src/core/strategies/bot-status.service.pure";
import {
  ILogger,
  ICacheService,
  CacheResult,
...
describe("BotStatusService", () => {
  ...
  describe("startBot", () => {
    it("should start a bot successfully when in valid state", async () => {
      ...
      expect(mockBotRepository.updateStatus).toHaveBeenCalledWith(
        mockBotId,
        "STARTING"
      );
    ...
    it("should handle errors when starting bot", async () => {
      (mockBotRepository.findById as jest.Mock).mockRejectedValue(
  ...
  describe("getBotStatusInfo", () => {
    ...
    it("should query repository when cache misses", async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        "Bot status cache miss, querying repository",
        expect.anything()
      );
    });
  ...
  describe("sendBotHeartbeat", () => {
    ...
    it("should handle recovery from RECOVERING state", async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        "Bot recovered from error state",
        expect.anything()
      );
    });
    ...
    it("should handle errors when processing heartbeat", async () => {
      (mockBotRepository.updateHeartbeat as jest.Mock).mockRejectedValue(
        new Error("Database error")
      );
  ...
  describe("getBotStats", () => {
    ...
    it("should return default stats when repository fails", async () => {
      (mockBotRepository.getBotStats as jest.Mock).mockRejectedValue(
        new Error("Database error")
      );
  ...
  describe("edge cases", () => {
    ...
    it("should handle audit logger failure gracefully", async () => {
      (mockAuditLogger.logEvent as jest.Mock).mockRejectedValue(
