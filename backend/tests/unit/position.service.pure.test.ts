import {
  ...
  PositionServiceDependencies,
  PositionValidationResult,
  AccountLimits,
  RiskAssessment,
} from "../../src/core/strategies/position.service.pure";
import {
  IPositionRepository,
  ICacheService,
  IExternalApiService,
...
describe("PositionService", () => {
  ...
  const mockBalance: Balance = new Balance(
    10000,
    5000,
    5000,
    "USD",
  ...
  describe("getUserPositions", () => {
    ...
    it("should fetch positions from repository when cache miss", async () => {
      ...
      expect(mockCache.get).toHaveBeenCalledWith(cacheKey);
      expect(mockPositionRepository.getPositions).toHaveBeenCalledWith(
        mockUserId
      );
    ...
    it("should handle cache set failure gracefully", async () => {
      ...
      (mockCache.setex as jest.Mock).mockResolvedValue({
        success: false,
  ...
  describe("getPosition", () => {
    ...
    it("should handle errors when fetching position", async () => {
      ...
      (mockPositionRepository.getPosition as jest.Mock).mockRejectedValue(
        testError
      );
  ...
  describe("calculateAccountLimits", () => {
    it("should calculate account limits correctly", async () => {
      ...
      const result = await service.calculateAccountLimits(
        mockUserId,
        mockBalance
      );
    ...
    it("should handle errors when calculating limits", async () => {
      ...
      (mockCache.get as jest.Mock).mockResolvedValue({ success: false });
      (mockPositionRepository.getPositions as jest.Mock).mockRejectedValue(
        testError
      );

      await expect(
        service.calculateAccountLimits(mockUserId, mockBalance)
  ...
  describe("assessRisk", () => {
    ...
    it("should assess high risk level correctly", async () => {
      ...
      const result = await service.assessRisk(
        mockUserId,
        new Balance(1000, 500, 500, "USD", new Date())
      );
    ...
    it("should identify concentrated positions", async () => {
      ...
      const result = await service.assessRisk(
        mockUserId,
        new Balance(25000, 12500, 12500, "USD", new Date())
      );

      expect(result.recommendations.some(rec => rec.includes("BTC/USDT"))).toBe(
        true
      );
    });

    it("should identify positions near liquidation", async () => {
      const nearLiquidationPosition = new Position(
        "BTC/USDT",
        "LONG",
        0.1,
        50000,
      ...
      (mockCache.get as jest.Mock).mockResolvedValue({ success: false });
      (mockPositionRepository.getPositions as jest.Mock).mockResolvedValue([
        nearLiquidationPosition,
      ]);

      const result = await service.assessRisk(mockUserId, mockBalance);

      expect(
        result.recommendations.some(rec => rec.includes("near liquidation"))
      ).toBe(true);
    });
  });
