...
} from "../../src/infrastructure/async/async-operation-manager.service";
import {
  getCurrentContext,
  createChildContext,
  RequestContext,
...
describe("AsyncOperationManager", () => {
  ...
  describe("executeWithContext", () => {
    it("should execute operation without context when none exists", async () => {
      ...
      const result = await manager.executeWithContext(
        "test-operation",
        mockOperation
      );
  ...
  describe("stats and management", () => {
    ...
    it("should track job statistics", async () => {
      ...
      manager.submitJob("job2", jest.fn().mockResolvedValue(undefined), {
        delayMs: 1000,
      });
