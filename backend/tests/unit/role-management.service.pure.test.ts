...
import {
  ...
  RoleManagementServiceDependencies,
} from "../../src/core/auth/role-management.service.pure";
...
describe("RoleManagementService", () => {
  ...
  describe("Role Assignment", () => {
    it("should successfully assign a role to a user", async () => {
      ...
      (deps.roleRepository.assignRole as jest.Mock).mockResolvedValue(
        undefined
      );
      ...
      expect(deps.roleRepository.hasRole).toHaveBeenCalledWith(
        testUserId,
        testRole
      );
      expect(deps.roleRepository.assignRole).toHaveBeenCalledWith(
        testUserId,
        testRole,
        testGrantedBy,
        undefined
    ...
    it("should handle role assignment errors", async () => {
      ...
      (deps.roleRepository.hasRole as jest.Mock).mockRejectedValue(
        new Error("Database error")
      );

      await expect(
        roleManagementService.assignRole(testUserId, testRole, testGrantedBy)
      ).rejects.toThrow();
  ...
  describe("Role Removal", () => {
    it("should successfully remove a role from a user", async () => {
      ...
      expect(deps.roleRepository.removeRole).toHaveBeenCalledWith(
        testUserId,
        testRole
      );
      expect(deps.auditLogger.logEvent).toHaveBeenCalledWith(
    ...
    it("should handle role removal errors", async () => {
      ...
      (deps.roleRepository.removeRole as jest.Mock).mockRejectedValue(
        new Error("Database error")
      );

      await expect(
        roleManagementService.removeRole(testUserId, testRole)
      ).rejects.toThrow();
      expect(deps.logger.error).toHaveBeenCalled();
    });
  });
  ...
  describe("User Roles", () => {
    ...
    it("should handle getting user roles errors", async () => {
      ...
      (deps.roleRepository.getUserRoles as jest.Mock).mockRejectedValue(
        new Error("Database error")
      );

      await expect(
        roleManagementService.getUserRoles(testUserId)
      ).rejects.toThrow();
      expect(deps.logger.error).toHaveBeenCalled();
    });
  });
  ...
  describe("Role Details", () => {
    it("should get role details for a user", async () => {
      ...
      (deps.roleRepository.getRoleDetails as jest.Mock).mockResolvedValue(
        testRoleDetails
      );

      const result = await roleManagementService.getRoleDetails(
        testUserId,
        testRole
      );

      expect(result).toEqual(testRoleDetails);
      expect(deps.roleRepository.getRoleDetails).toHaveBeenCalledWith(
        testUserId,
        testRole
      );
    });
    ...
    it("should handle getting role details errors", async () => {
      ...
      (deps.roleRepository.getRoleDetails as jest.Mock).mockRejectedValue(
        new Error("Database error")
      );
  ...
  describe("Users with Role", () => {
    it("should get all users with specific role", async () => {
      ...
      (deps.roleRepository.getUsersWithRole as jest.Mock).mockResolvedValue(
        testUsers
      );
      ...
      expect(deps.roleRepository.getUsersWithRole).toHaveBeenCalledWith(
        testRole
      );
    });
    ...
    it("should handle getting users with role errors", async () => {
      ...
      (deps.roleRepository.getUsersWithRole as jest.Mock).mockRejectedValue(
        new Error("Database error")
      );
  ...
  describe("Role Revalidation", () => {
    it("should revalidate role successfully when qualification passes", async () => {
      ...
      (
        deps.qualificationService?.checkQualification as jest.Mock
      ).mockResolvedValue({ qualified: true });

      const result = await roleManagementService.revalidateRole(
        testUserId,
        testRole
      );
      ...
      expect(result).toBe(true);
      expect(
        deps.qualificationService?.checkQualification
      ).toHaveBeenCalledWith(testUserId, testRole);
    });
  ...
  describe("Permission Check", () => {
    it("should return true when user has permission", async () => {
      ...
      const result = await roleManagementService.hasPermission(
        testUserId,
        testRequiredRole
      );
  ...
  describe("Role Level Management", () => {
    it("should get user highest role level", async () => {
      ...
      const result =
        await roleManagementService.getUserHighestRoleLevel(testUserId);
