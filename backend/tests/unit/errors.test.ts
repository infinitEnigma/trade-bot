...
describe("Error System", () => {
  ...
  describe("Business Logic Errors", () => {
    ...
    it("should create PositionSizeExceededError instance", () => {
      ...
      const error = new PositionSizeExceededError(
        requested,
        maxAllowed,
        context
      );
  ...
  describe("Data Freshness Utils", () => {
    ...
    it("should create API metadata", () => {
      ...
      const lastUpdated = Date.now() - 1000;
      const metadata = DataFreshnessUtils.createApiMetadata(
        updateFrequency,
        lastUpdated
      );
      ...
      expect(metadata.recommendedPollInterval).toBe(
        Math.max(updateFrequency * 2, 30000)
      );
    ...
    it("should create cache metadata", () => {
      ...
      const lastUpdated = Date.now() - 1000;
      const metadata = DataFreshnessUtils.createCacheMetadata(
        cacheTTL,
        lastUpdated
      );
