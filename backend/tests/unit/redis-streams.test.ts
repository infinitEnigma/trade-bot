...
describe("RedisStreamOperations", () => {
  ...
  describe("publish method", () => {
    it("should publish message to stream successfully", async () => {
      ...
      expect(mockClient.xAdd).toHaveBeenCalledWith("test:stream", "*", {
  ...
  describe("ack method", () => {
    it("should acknowledge single message", async () => {
      ...
      const result = await streamOperations.ack(
        "test:stream",
        "test-group",
        "123"
      );
    ...
    it("should acknowledge multiple messages", async () => {
      ...
      const result = await streamOperations.ack("test:stream", "test-group", [
        "123",
        "456",
      ]);
  ...
  describe("createConsumerGroup method", () => {
    it("should create consumer group successfully", async () => {
      ...
      const result = await streamOperations.createConsumerGroup(
        "test:stream",
        "test-group"
      );
    ...
    it("should handle existing consumer group", async () => {
      mockClient.xGroupCreate.mockRejectedValue(
  ...
  describe("trim method", () => {
    it("should trim stream successfully with approximate true", async () => {
      mockClient.xInfoStream
        .mockResolvedValueOnce({ length: 20 }) // Before trim
  ...
  describe("getPendingInsight method", () => {
      ...
      mockClient.xPendingRange.mockResolvedValue([
        {
          ...
          millisecondsSinceLastDelivery: 5_000,
          deliveriesCounter: 1,
        },
        {
          id: "2-0",
          consumer: "backend-consumer",
          millisecondsSinceLastDelivery: 60_000,
          deliveriesCounter: 12,
        },
        {
          id: "3-0",
          consumer: "engine-consumer",
          millisecondsSinceLastDelivery: 45_000,
          deliveriesCounter: 3,
        },
      ]);

      const insight = await streamOperations.getPendingInsight(
        "test:stream",
        "test-group",
        INSIGHT_OPTS
      );

      expect(mockClient.xPendingRange).toHaveBeenCalledWith(
        "test:stream",
        "test-group",
        "-",
        "+",
    ...
    it("should propagate client errors to the caller", async () => {
      ...
      await expect(
        streamOperations.getPendingInsight(
          "test:stream",
          "test-group",
          INSIGHT_OPTS
        )
