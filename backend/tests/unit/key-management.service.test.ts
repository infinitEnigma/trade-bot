...
import {
  ...
  KeyPurpose,
  keyManagementService,
} from "../../src/infrastructure/security/key-management.service";
...
describe("KeyManagementService", () => {
  ...
  describe("error handling", () => {
    it("should throw error for invalid encrypted data", async () => {
      const keyManagement = new KeyManagementService();
      await expect(
        keyManagement.decrypt("invalid-base64-data")
      ).rejects.toThrow();
    });
    ...
    it("should throw error for unsupported key version", async () => {
      ...
      const keyManagement = new KeyManagementService();
      await expect(keyManagement.decrypt(invalidData)).rejects.toThrow(
    ...
    it("should handle key derivation failure", async () => {
      ...
      const spy = jest
        .spyOn(KeyManagementService.prototype as any, "derivePurposeKeys")
        .mockRejectedValue(new Error("Key derivation failed"));
      const keyManagement = new KeyManagementService();

      await expect(keyManagement.encrypt("test")).rejects.toThrow(
