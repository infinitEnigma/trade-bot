...
import {
  hashPassword,
  comparePassword,
} from "../../src/workers/password-worker";
...
describe("PasswordAdapter", () => {
  ...
  describe("hash", () => {
    ...
    it("should throw error when password is empty", async () => {
      await expect(passwordAdapter.hash("")).rejects.toThrow(
    ...
    it("should throw error when rounds are less than 8", async () => {
      await expect(passwordAdapter.hash("TestPassword1!", 7)).rejects.toThrow(
        "Bcrypt rounds must be between 8 and 20"
      );
      expect(hashPassword).not.toHaveBeenCalled();
    });

    it("should throw error when rounds are greater than 20", async () => {
      await expect(passwordAdapter.hash("TestPassword1!", 21)).rejects.toThrow(
        "Bcrypt rounds must be between 8 and 20"
      );
      expect(hashPassword).not.toHaveBeenCalled();
    });
    ...
    it("should throw error with message when hashing fails", async () => {
      ...
      await expect(passwordAdapter.hash(password)).rejects.toThrow(
