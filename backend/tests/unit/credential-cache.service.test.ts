...
describe("CredentialCacheService", () => {
  ...
  describe("instance creation", () => {
    it("should export a singleton instance", () => {
      ...
      expect(typeof credentialCacheService.getCachedCredentials).toBe(
        "function"
      );
      expect(typeof credentialCacheService.cacheCredentials).toBe("function");
      expect(typeof credentialCacheService.getOrCacheCredentials).toBe(
        "function"
      );
      expect(typeof credentialCacheService.invalidateCredentials).toBe(
        "function"
      );
  ...
  describe("getCachedCredentials", () => {
    ...
    it("should return null and delete entry for expired cache", () => {
      ...
      (encryptionService.decryptApiKey as jest.Mock) = jest
        .fn()
        .mockReturnValue("test-api-key");
      (encryptionService.decryptSecretKey as jest.Mock) = jest
        .fn()
        .mockReturnValue("test-secret-key");
  ...
  describe("cache operations", () => {
    it("should handle multiple users independently", async () => {
      ...
      const user1Result =
        credentialCacheService.getCachedCredentials("user-123");
      const user2Result =
        credentialCacheService.getCachedCredentials("user-456");
