...
import {
  ...
  RETRY_CONDITIONS,
  RetryResult,
} from "../../src/infrastructure/retry.service";
...
describe("RetryService", () => {
  ...
  beforeEach(() => {
    ...
    jest
      .spyOn(retryServiceInstance as any, "delay")
  ...
  describe("executeWithRetry", () => {
    it("should succeed on first attempt", async () => {
      ...
      const result = await retryServiceInstance.executeWithRetry(
        mockOperation,
        operationName
      );
    ...
    it("should notify with high severity on final failure", async () => {
      ...
      await retryServiceInstance.executeWithRetry(
        mockOperation,
        operationName,
        {
          maxAttempts: 2,
  ...
  describe("executeWithCircuitBreaker", () => {
    it("should delegate to executeWithRetry", async () => {
      ...
      await retryServiceInstance.executeWithCircuitBreaker(
        mockOperation,
        operationName,
        circuitBreakerKey
      );
  ...
  describe("withRetry convenience function", () => {
    ...
    it("should use custom retry configuration", async () => {
      ...
      await expect(
        withRetry(mockOperation, operationName, RETRY_CONFIGS.FAST)
      ).rejects.toThrow();
      expect(mockOperation).toHaveBeenCalledTimes(3);
    });
  });
  ...
  describe("RETRY_CONFIGS", () => {
    it("should export predefined retry configurations", () => {
      expect(RETRY_CONFIGS.FAST).toEqual(
      ...
      expect(RETRY_CONFIGS.STANDARD).toEqual(
        expect.objectContaining({
          maxAttempts: 3,
          baseDelay: 1000,
          maxDelay: 10000,
      ...
      expect(RETRY_CONFIGS.SLOW).toEqual(
        expect.objectContaining({
          maxAttempts: 5,
          baseDelay: 2000,
          maxDelay: 30000,
      ...
      expect(RETRY_CONFIGS.AGGRESSIVE).toEqual(
        expect.objectContaining({
          maxAttempts: 5,
          baseDelay: 1000,
          maxDelay: 60000,
  ...
  describe("isDefaultRetryableError", () => {
    ...
    it("should recognize HTTP 5xx errors as retryable", async () => {
      const operation = jest
        .fn()
        .mockRejectedValue(new Error("Status code 503"));
