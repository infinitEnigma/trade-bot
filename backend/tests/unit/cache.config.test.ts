...
describe("CacheConfig", () => {
  ...
  describe("Data freshness configuration", () => {
    it("should have valid data freshness properties", () => {
      ...
      expect(DATA_FRESHNESS.MARKET_HIGH_FREQ).toBeGreaterThan(
        DATA_FRESHNESS.MARKET_REALTIME
      );
      expect(DATA_FRESHNESS.MARKET_MEDIUM_FREQ).toBeGreaterThan(
        DATA_FRESHNESS.MARKET_HIGH_FREQ
      );
      expect(DATA_FRESHNESS.MARKET_LOW_FREQ).toBeGreaterThan(
        DATA_FRESHNESS.MARKET_MEDIUM_FREQ
      );
      expect(DATA_FRESHNESS.MARKET_STATIC).toBeGreaterThan(
        DATA_FRESHNESS.MARKET_LOW_FREQ
      );

      expect(DATA_FRESHNESS.POLL_REALTIME).toBeGreaterThan(0);
      expect(DATA_FRESHNESS.POLL_HIGH_FREQ).toBeGreaterThan(
        DATA_FRESHNESS.POLL_REALTIME
      );
      expect(DATA_FRESHNESS.POLL_MEDIUM_FREQ).toBeGreaterThan(
        DATA_FRESHNESS.POLL_HIGH_FREQ
      );
      expect(DATA_FRESHNESS.POLL_LOW_FREQ).toBeGreaterThan(
        DATA_FRESHNESS.POLL_MEDIUM_FREQ
      );
      expect(DATA_FRESHNESS.POLL_STATIC).toBeGreaterThan(
        DATA_FRESHNESS.POLL_LOW_FREQ
      );

      expect(DATA_FRESHNESS.STALE_THRESHOLD_REALTIME).toBeGreaterThan(
        DATA_FRESHNESS.POLL_REALTIME
      );
      expect(DATA_FRESHNESS.STALE_THRESHOLD_HIGH_FREQ).toBeGreaterThan(
        DATA_FRESHNESS.POLL_HIGH_FREQ
      );
      expect(DATA_FRESHNESS.STALE_THRESHOLD_MEDIUM_FREQ).toBeGreaterThan(
        DATA_FRESHNESS.POLL_MEDIUM_FREQ
      );
      expect(DATA_FRESHNESS.STALE_THRESHOLD_LOW_FREQ).toBeGreaterThan(
        DATA_FRESHNESS.POLL_LOW_FREQ
      );
  ...
  describe("TTL key mapping", () => {
    ...
    it("should return correct TTL for market data keys", () => {
      ...
      expect(getTTLForKey(CACHE_KEYS.markPrice("BTCUSDT"))).toBe(
        config.MARKET_MARK_PRICE
      );
      expect(getTTLForKey(CACHE_KEYS.futures("BTCUSDT"))).toBe(
        config.MARKET_FUTURES
      );
      expect(getTTLForKey(CACHE_KEYS.tradingViewConfig())).toBe(
        config.MARKET_TRADINGVIEW_CONFIG
      );
    });
    ...
    it("should return correct TTL for rate limiting keys", () => {
      ...
      expect(getTTLForKey(CACHE_KEYS.rateLimit("auth", "127.0.0.1"))).toBe(
        config.RATE_LIMIT_AUTH
      );
      expect(getTTLForKey(CACHE_KEYS.rateLimit("market", "127.0.0.1"))).toBe(
        config.RATE_LIMIT_MARKET
      );
      expect(getTTLForKey(CACHE_KEYS.rateLimit("trading", "127.0.0.1"))).toBe(
        config.RATE_LIMIT_TRADING
      );
      expect(getTTLForKey(CACHE_KEYS.rateLimit("general", "127.0.0.1"))).toBe(
    ...
    it("should return correct TTL for application data keys", () => {
      ...
      expect(getTTLForKey(CACHE_KEYS.credential("user123"))).toBe(
        config.CREDENTIAL_CACHE
      );
      expect(getTTLForKey(CACHE_KEYS.position("user123"))).toBe(
        config.POSITION_CACHE
      );
      expect(getTTLForKey(CACHE_KEYS.balance("user123"))).toBe(
        config.BALANCE_CACHE
      );
    });
