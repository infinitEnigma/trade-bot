...
describe("StrategyService", () => {
  ...
  describe("getStrategies", () => {
    it("should retrieve all strategies for a user", async () => {
      ...
      expect(deps.strategyRepository.getStrategies).toHaveBeenCalledWith(
        testUserId
      );
    ...
    it("should handle errors when retrieving strategies", async () => {
      ...
      (deps.strategyRepository.getStrategies as jest.Mock).mockRejectedValue(
        testError
      );

      await expect(strategyService.getStrategies(testUserId)).rejects.toThrow(
  ...
  describe("getStrategy", () => {
    it("should retrieve a specific strategy by ID", async () => {
      ...
      expect(deps.strategyRepository.getStrategy).toHaveBeenCalledWith(
        testStrategyId
      );
      expect(deps.logger.debug).toHaveBeenCalled();
    });
    ...
    it("should handle errors when retrieving strategy", async () => {
      ...
      const testStrategyId = "strategy-123";
      const testError = new Error("Database error");
      (deps.strategyRepository.getStrategy as jest.Mock).mockRejectedValue(
        testError
      );

      await expect(strategyService.getStrategy(testStrategyId)).rejects.toThrow(
  ...
  describe("createStrategy", () => {
    it("should create a new strategy", async () => {
      ...
      expect(result).toEqual(mockStrategy);
      expect(deps.strategyRepository.createStrategy).toHaveBeenCalledWith(
    ...
    it("should handle errors when creating strategy", async () => {
      ...
      await expect(
        strategyService.createStrategy(testUserId, testStrategyData)
  ...
  describe("updateStrategy", () => {
    it("should update an existing strategy", async () => {
      ...
      expect(deps.strategyRepository.updateStrategy).toHaveBeenCalledWith(
        testStrategyId,
        testUpdates
      );
      expect(deps.strategyRepository.getStrategy).toHaveBeenCalledWith(
    ...
    it("should handle errors when updating strategy", async () => {
      ...
      await expect(
        strategyService.updateStrategy(testStrategyId, testUpdates)
      ).rejects.toThrow("Failed to update strategy");
      expect(deps.logger.error).toHaveBeenCalled();
    });
  });
  ...
  describe("deleteStrategy", () => {
    it("should delete a strategy with associated bot instances", async () => {
      ...
      mockBotInstances.forEach(bot => {
        expect(
          deps.botInstanceRepository.deleteBotInstance
        ).toHaveBeenCalledWith(bot.id);
      });
      expect(deps.strategyRepository.deleteStrategy).toHaveBeenCalledWith(
        testStrategyId
      );
      expect(deps.logger.info).toHaveBeenCalled();
    });
    ...
    it("should handle errors when deleting strategy", async () => {
      ...
      await expect(
        strategyService.deleteStrategy(testStrategyId)
      ).rejects.toThrow("Failed to delete strategy");
      expect(deps.logger.error).toHaveBeenCalled();
    });
  });
  ...
  describe("toggleStrategy", () => {
    it("should toggle strategy active status to active", async () => {
      ...
      const testStrategyId = "strategy-123";
      (deps.strategyRepository.toggleStrategy as jest.Mock).mockResolvedValue(
        undefined
      );

      await strategyService.toggleStrategy(testStrategyId, true);

      expect(deps.strategyRepository.toggleStrategy).toHaveBeenCalledWith(
        testStrategyId,
        true
      );
      expect(deps.logger.info).toHaveBeenCalled();
    ...
    it("should handle errors when toggling strategy", async () => {
      ...
      (deps.strategyRepository.toggleStrategy as jest.Mock).mockRejectedValue(
        testError
      );

      await expect(
        strategyService.toggleStrategy(testStrategyId, true)
      ).rejects.toThrow("Failed to toggle strategy");
      expect(deps.logger.error).toHaveBeenCalled();
    });
  });
  ...
  describe("getStrategyPerformance", () => {
    it("should retrieve strategy performance metrics", async () => {
      ...
      const testStrategyId = "strategy-123";
      const result =
        await strategyService.getStrategyPerformance(testStrategyId);
    ...
    it("should handle errors when retrieving strategy performance", async () => {
      ...
      (strategyService as any).getStrategyPerformance = jest
        .fn()
        .mockRejectedValue(new Error("Performance data unavailable"));

      await expect(
        (strategyService as any).getStrategyPerformance("strategy-123")
      ).rejects.toThrow("Performance data unavailable");
    });
  });
});
