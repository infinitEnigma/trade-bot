import {
  ...
  ProcessState,
  SupervisorConfig,
} from "../../src/core/strategies/engine/process-supervisor";
import { ProcessSpawner } from "../../src/core/strategies/engine/process-spawner";
import {
  HealthMonitor,
  EngineHealth,
} from "../../src/core/strategies/engine/health-monitor";
import {
  RestartManager,
  RestartPolicy,
...
import {
  CircuitBreaker,
  CircuitState,
} from "../../src/core/strategies/engine/circuit-breaker";
...
describe("ProcessSupervisor", () => {
  ...
  describe("constructor", () => {
    ...
    it("should create a ProcessSupervisor instance with custom configuration", () => {
      ...
      expect(supervisor.getSupervisorStatus().config).toEqual(
  ...
  describe("basic functionality", () => {
    it("should start and stop supervision", async () => {
      ...
      const superviseProcessLifecycle = (ProcessSupervisor.prototype as any)
        .superviseProcessLifecycle;
      await superviseProcessLifecycle.call(processSupervisor);

      expect(processSupervisor.getProcessState()).not.toEqual(
  ...
  describe("state management", () => {
    ...
    it("should transition to RECOVERING when health check fails and auto-restart is enabled", async () => {
      healthMonitor.performMultiLayerHealthCheck.mockResolvedValue(
        mockUnhealthy
      );
  ...
  describe("recovery mechanisms", () => {
    it("should attempt restart when process crashes and auto-restart is enabled", async () => {
      ...
      await new Promise<void>(resolve => {
        const originalHandleProcessCrash = (ProcessSupervisor.prototype as any)
          .handleProcessCrash;
        (ProcessSupervisor.prototype as any).handleProcessCrash =
          async function (health: EngineHealth) {
      ...
      expect(restartManager.attemptIntelligentRestart).toHaveBeenCalledWith(
        "process_crash"
      );
      expect(processSupervisor.getProcessState()).toBe(ProcessState.RECOVERING);
    ...
    it("should handle failed restart attempts", async () => {
      ...
      await new Promise<void>(resolve => {
        const originalHandlePermanentFailure = (
          ProcessSupervisor.prototype as any
        ).handlePermanentFailure;
        (ProcessSupervisor.prototype as any).handlePermanentFailure =
    ...
    it("should handle unhealthy process and attempt recovery", async () => {
      ...
      await new Promise<void>(resolve => {
        const originalHandleProcessUnhealthy = (
          ProcessSupervisor.prototype as any
        ).handleProcessUnhealthy;
        (ProcessSupervisor.prototype as any).handleProcessUnhealthy =
          async function (health: EngineHealth) {
  ...
  describe("supervision lifecycle", () => {
    it("should handle supervision failures gracefully", async () => {
      healthMonitor.performMultiLayerHealthCheck.mockRejectedValue(
    ...
    it("should track supervision cycles", async () => {
      const initialCycles =
        processSupervisor.getSupervisionStats().supervisionCycles;
