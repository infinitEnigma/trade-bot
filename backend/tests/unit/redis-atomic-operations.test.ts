...
describe("RedisAtomicOperations", () => {
  ...
  beforeEach(() => {
    ...
    mockConnectionManager =
      new RedisConnectionManager() as jest.Mocked<RedisConnectionManager>;
    mockTransactions = new RedisTransactions(
      mockConnectionManager
    ) as jest.Mocked<RedisTransactions>;

    // Initialize the class under test
    atomicOperations = new RedisAtomicOperations(
      mockConnectionManager,
      mockTransactions
    );
  ...
  describe("atomicIncrementWithExpiry", () => {
    it("should perform atomic increment successfully without expiry", async () => {
      ...
      const result = await atomicOperations.atomicIncrementWithExpiry(
        mockKey,
        mockIncrement
      );
  ...
  describe("atomic operations with specific scenarios", () => {
    it("should handle conditional update when value is null", async () => {
      ...
      (mockConnectionManager.getClient().get as jest.Mock).mockResolvedValue(
        null
      );

      const result = await atomicOperations.atomicConditionalUpdate(
        mockKey,
        mockNewValue,
        mockExpectedValue
      );
    ...
    it("should handle balance transfer with zero initial balances", async () => {
      ...
      const result = await atomicOperations.atomicBalanceTransfer(
        mockFromKey,
        mockToKey,
        mockAmount,
        false
    ...
    it("should handle versioned update without expected version", async () => {
      ...
      const result = await atomicOperations.atomicVersionedUpdate(
        mockDataKey,
        mockNewData
      );
  ...
  describe("atomicConditionalUpdate", () => {
    it("should perform conditional update successfully when values match", async () => {
      ...
      (mockConnectionManager.getClient().get as jest.Mock).mockResolvedValue(
        JSON.stringify(mockExpectedValue)
      );
  ...
  describe("atomicReadModifyWrite", () => {
    it("should perform successful read-modify-write operation", async () => {
      ...
      mockTransactions.watchMultiExec.mockImplementation(
        async (_, operation) => {
      ...
      // Mock getting current value
      (mockConnectionManager.getClient().get as jest.Mock).mockResolvedValue(
        JSON.stringify(mockCurrentValue)
      );

      const result = await atomicOperations.atomicReadModifyWrite(
        mockKey,
        mockModifier
      );
    ...
    it("should perform successful optimistic update", async () => {
      ...
      mockTransactions.watchMultiExec.mockImplementation(
        async (_, operation) => {
          ...
          return {
            success: true,
            result: { newData: mockNewData, version: 2 },
          };
        }
      );
      ...
      const result = await atomicOperations.atomicOptimisticUpdate(
        mockKey,
        mockUpdateFunction
      );
