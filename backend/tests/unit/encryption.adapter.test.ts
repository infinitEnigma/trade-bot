...
describe("EncryptionAdapter", () => {
  ...
  describe("encryptApiKey", () => {
    it("should encrypt API key using encryption service", () => {
      ...
      expect(integrationLogger.startOperation).toHaveBeenCalledWith(
        "encryptApiKey",
        {
    ...
    it("should throw error when encrypting empty API key", () => {
      expect(() => encryptionAdapter.encryptApiKey("")).toThrow(
    ...
    it("should handle encryption service errors", () => {
      ...
      expect(() => encryptionAdapter.encryptApiKey(apiKey)).toThrow(
        `API key encryption failed: ${error.message}`
      );
  ...
  describe("decryptApiKey", () => {
    ...
    it("should throw error when decrypting empty API key", () => {
      expect(() => encryptionAdapter.decryptApiKey("")).toThrow(
    ...
    it("should handle decryption service errors", () => {
      ...
      expect(() => encryptionAdapter.decryptApiKey(encryptedApiKey)).toThrow(
        `API key decryption failed: ${error.message}`
      );
      expect(integrationLogger.error).toHaveBeenCalledWith(
  ...
  describe("encryptSecretKey", () => {
    it("should encrypt secret key using encryption service", () => {
      ...
      expect(encryptionService.encryptSecretKey).toHaveBeenCalledWith(
        secretKey
      );
      expect(integrationLogger.startOperation).toHaveBeenCalledWith(
        "encryptSecretKey",
    ...
    it("should throw error when encrypting empty secret key", () => {
      expect(() => encryptionAdapter.encryptSecretKey("")).toThrow(
    ...
    it("should handle encryption service errors", () => {
      ...
      expect(() => encryptionAdapter.encryptSecretKey(secretKey)).toThrow(
        `Secret key encryption failed: ${error.message}`
      );
      expect(integrationLogger.error).toHaveBeenCalledWith(
  ...
  describe("decryptSecretKey", () => {
    it("should decrypt secret key using encryption service", () => {
      ...
      expect(encryptionService.decryptSecretKey).toHaveBeenCalledWith(
        encryptedSecretKey
      );
      expect(integrationLogger.startOperation).toHaveBeenCalledWith(
        "decryptSecretKey",
    ...
    it("should throw error when decrypting empty secret key", () => {
      expect(() => encryptionAdapter.decryptSecretKey("")).toThrow(
        "Encrypted secret key cannot be empty"
      );
    ...
    it("should handle decryption service errors", () => {
      ...
      expect(() =>
        encryptionAdapter.decryptSecretKey(encryptedSecretKey)
      ).toThrow(`Secret key decryption failed: ${error.message}`);
      expect(integrationLogger.error).toHaveBeenCalledWith(
  ...
  describe("encryptWithVersion", () => {
    it("should encrypt data with version using encryption service", async () => {
      ...
      const result = await encryptionAdapter.encryptWithVersion(
        plaintext,
        version
      );

      expect(encryptionService.encryptWithVersion).toHaveBeenCalledWith(
        plaintext,
        version
      );
      expect(integrationLogger.startOperation).toHaveBeenCalledWith(
    ...
    it("should throw error when encrypting empty plaintext", async () => {
      await expect(encryptionAdapter.encryptWithVersion("")).rejects.toThrow(
    ...
    it("should handle encryption service errors", async () => {
      ...
      (encryptionService.encryptWithVersion as jest.Mock).mockRejectedValue(
        error
      );

      await expect(
        encryptionAdapter.encryptWithVersion(plaintext)
      ).rejects.toThrow(`Versioned encryption failed: ${error.message}`);
      expect(integrationLogger.error).toHaveBeenCalledWith(
  ...
  describe("decryptWithVersion", () => {
    it("should decrypt data with version using encryption service", async () => {
      ...
      expect(encryptionService.decryptWithVersion).toHaveBeenCalledWith(
        encryptedData
      );
      expect(integrationLogger.startOperation).toHaveBeenCalledWith(
        "decryptWithVersion",
    ...
    it("should throw error when decrypting empty data", async () => {
      await expect(encryptionAdapter.decryptWithVersion("")).rejects.toThrow(
    ...
    it("should handle decryption service errors", async () => {
      ...
      await expect(
        encryptionAdapter.decryptWithVersion(encryptedData)
      ).rejects.toThrow(`Versioned decryption failed: ${error.message}`);
      expect(integrationLogger.error).toHaveBeenCalledWith(
  ...
  describe("isKeyRotationNeeded", () => {
    it("should check if key rotation is needed", async () => {
      ...
      (encryptionService.isKeyRotationNeeded as jest.Mock).mockResolvedValue(
        rotationNeeded
      );
    ...
    it("should return false and log warning when check fails", async () => {
      ...
      (encryptionService.isKeyRotationNeeded as jest.Mock).mockRejectedValue(
        error
      );
  ...
  describe("rotateEncryptionKeys", () => {
    it("should rotate encryption keys", async () => {
      (encryptionService.rotateEncryptionKeys as jest.Mock).mockResolvedValue(
        undefined
      );
    ...
    it("should handle key rotation errors", async () => {
      ...
      (encryptionService.rotateEncryptionKeys as jest.Mock).mockRejectedValue(
        error
      );

      await expect(encryptionAdapter.rotateEncryptionKeys()).rejects.toThrow(
