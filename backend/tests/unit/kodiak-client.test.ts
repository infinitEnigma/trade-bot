...
describe("KodiakClient", () => {
  ...
  const mockSignatureGeneration = () => {
    (client as any).generateSignature = jest
      .fn()
      .mockResolvedValue("mock-signature-12345");
  };
  ...
  describe("testConnectivity", () => {
    ...
    it("should return error for invalid credentials", async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        "Kodiak API connectivity test failed",
        {
          accountId: "test-account",
          error: "Invalid API key",
        }
  ...
  describe("handleApiError", () => {
    it("should handle JSON error response", () => {
      const result = (client as any).handleApiError(
        400,
