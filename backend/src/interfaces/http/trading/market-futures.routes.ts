...
futuresRoutes.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      marketLogger.debug("Futures data cache miss, fetching from Kodiak", {
        symbol,
      });
      ...
      const stale = await readCache<Record<string, unknown>>(
        `futures:${req.params.symbol}`,
        {
          symbol: req.params.symbol,
        }
      );
...
futuresRoutes.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const response =
        await kodiakIntegrationService.getMarketTicker(symbolStr);
      if (response.success && response.data) {
        const markPrice = parseFloat(
          (
            response.data as { mark_price?: string | number }
          ).mark_price?.toString() || "0"
        );
        if (markPrice > 0) {
          ok(
            res,
