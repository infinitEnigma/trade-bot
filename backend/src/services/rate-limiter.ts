    ...
    try {
      ...
      if (redisAvailable) {
        try {
          ...
          resetTime = ttlResult > 0 ? Date.now() + ttlResult : Date.now() + config.windowMs;

        } catch (redisError) {
          logger.warn('Redis rate limiting failed, switching to in-memory fallback', {
            endpoint,
            ip: req.ip,
          ...
          const memoryResult = memoryRateLimiter.check(key, config.max, config.windowMs);
