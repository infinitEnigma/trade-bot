...
// Using proper Ed25519 signing as in the working example
async function generateKodiakSignature(
  ...
  // Use proper Ed25519 signing (as in working example)
  const signature = await ed25519.sign(messageBytes, privateKey);

  // Convert to base64url format (as in working example)
...
async function makeKodiakRequest(
  ...
  // Remove chain-id entirely - it might not be supported by this API
  console.log(`Not adding orderly-chain-id header for path: "${path}"`);
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          if (
            (publicData as any).success &&
            (publicData as any).data?.address
          ) {
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (tradesData.success && tradesData.data) {
        ...
        res
          .status(400)
          .json({ success: false, error: "Failed to fetch trades" });
      }
    } catch (err) {
