...
export class ExternalApiAdapter implements IExternalApiService {
    ...
    try {
      ...
      if (!kodiakResult.success) {
        return {
          success: false,
          error:
            kodiakResult.error || "Failed to get balance from external API",
          timestamp: Date.now(),
        };
      }
    ...
    try {
      ...
      if (!kodiakResult.success) {
        return {
          success: false,
          error:
            kodiakResult.error || "Failed to get positions from external API",
          timestamp: Date.now(),
        };
      }
      ...
      const kodiakPositions = Array.isArray(kodiakResult.data)
        ? kodiakResult.data
        : [];
  ...
  async getTrades(
    ...
    try {
      const kodiakResult = await kodiakIntegrationService.getTrades(
        userId,
        limit
      );
      ...
      const kodiakTrades = Array.isArray(kodiakResult.data)
        ? kodiakResult.data
        : [];
  ...
  async getAccountInfo(userId: string): Promise<ApiResult<AccountInfo>> {
    try {
      const kodiakResult =
        await kodiakIntegrationService.getAccountInfo(userId);
      ...
      if (!kodiakResult.success) {
        return {
          ...
          error:
            kodiakResult.error ||
            "Failed to get account info from external API",
          timestamp: Date.now(),
        };
      }

      // Convert Kodiak account info to domain AccountInfo
      const kodiakAccountInfoResponse = kodiakResult.data as
        KodiakApiAccountInfoResponse | undefined;
      if (!kodiakAccountInfoResponse) {
        logger.warn("Kodiak account info data is null or undefined", {
          userId,
        });
        return {
          success: false,
      ...
      const kodiakAccountInfo: KodiakAccountInfo = {
        ...
        tradingVolume24H:
          kodiakAccountInfoResponse.trading_volume_last_24_hours || "0",
  ...
  async testConnectivity(
    ...
    try {
      ...
      const result =
        await kodiakIntegrationService.testConnectivity(kodiakCredentials);
  ...
  async validateWalletChain(
    ...
    try {
      ...
      logger.debug("Wallet chain validation", {
        walletAddress,
        requiredChain: chainId,
      });
      return true; // Placeholder - implement actual chain validation
    } catch (error) {
      logger.error("Wallet chain validation failed", error as Error, {
        walletAddress,
  ...
  async checkNFTOwnership(
    ...
    try {
      ...
      logger.debug("Checking NFT ownership", {
        walletAddress,
        contractAddress,
      });
  ...
  async checkTokenBalance(
    ...
    try {
      ...
      logger.debug("Checking token balance", {
        walletAddress,
        tokenAddress,
