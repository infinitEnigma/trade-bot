 ...
 */
export type RedisOperationCallback<T> = (
  multi: RedisMultiCommand
) => Promise<T>;
...
class RedisService {
  ...
  private constructor() {
    ...
    this.atomicOps = new RedisAtomicOperations(
  ...
  public async atomicCacheUpdate(
    ...
    const result = await this.cacheManager.atomicCacheUpdate(
      key,
      data,
      versionKey,
      maxRetries
  ...
  public async getWithVersion(
    ...
    const result = await this.cacheManager.getWithVersion<unknown>(
      key,
      versionKey
    );
    return {
  ...
  public async atomicIncrementWithExpiry(
    ...
    const result = await this.atomicOps.atomicIncrementWithExpiry(
      key,
      increment,
      ttlMs,
      {
  ...
  public async atomicConditionalUpdate(
    ...
    const result = await this.atomicOps.atomicConditionalUpdate(
      key,
      newValue,
      expectedValue,
      {
  ...
  public async atomicReadModifyWrite<T>(
    ...
    const result = await this.atomicOps.atomicReadModifyWrite<T>(
      key,
      modifier,
      defaultValue,
      {
  ...
  public async atomicBalanceTransfer(
    ...
    const result = await this.atomicOps.atomicBalanceTransfer(
      fromKey,
      toKey,
      amount,
  ...
  public async atomicVersionedUpdate(
    ...
    const result = await this.atomicOps.atomicVersionedUpdate(
      dataKey,
      newData,
      expectedVersion,
      versionKey,
  ...
  public async atomicCompositeUpdate(
    ...
    success: boolean;
    results?: Array<{ key: string; operation: string; value: unknown }>;
    error?: string;
  }> {
