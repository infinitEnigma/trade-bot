...
const CONFIG = {
  ...
  wsUrl:
    "wss://ws-evm.orderly.org/ws/stream/0x125a4423b9883442c1d53b5c8a88e815c8f0e69feaf0d7da699e0fff85eb2c54",
...
ws.on("message", data => {
  ...
  try {
    ...
    if (message.event && ["subscribe", "unsubscribe"].includes(message.event)) {
      if (message.success) {
        console.log(
          `✅ Subscription successful: ${message.event} ${message.topic}`
        );
      } else {
        console.error(
          `❌ Subscription failed: ${message.event} ${message.topic} - ${message.errorMsg}`
        );
      }
    }
